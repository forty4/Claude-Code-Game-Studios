...
- **Gap**: 기존 `hidden_condition` 은 `{type: fate_threshold, field: formation_turns, op: >=, value: 3}` — `formation_turns` 가 fate_data 에 emit 은 되지만 (grid_battle_controller.gd:3355), `_fate_formation_turns` 의 wiring (도원 형제 인접 대형 N 턴 유지 mechanic) 가 현재 미구현 일 가능성 있음 (spec impl 단계 의 검증 사항). 또한 plan §4.2 의 design intent ("결의의 timing — 빠른 재집결") 과 정합 약함 (formation 유지 ≠ timing).

본 spec 은 design intent + substrate 정합 모두 close — **`win_within_turns` field 채택**. `_fate_win_within_turns` 는 grid_battle_controller.gd:3347 에서 WIN outcome 시 자동 set (`VICTORY_REACH_TILE` 포함 — line 3343 의 outcome 매칭). 즉 ch08 의 REACH_TILE WIN 시 `_fate_win_within_turns = round_now` 가 자동 set 되어 fate_data 에 emit. ★ trigger `win_within_turns <= 6` 의 substrate 가 **완벽한 기존 wiring** — entity-less, 새 mechanic 0, 새 substrate 0.
...
`win_within_turns` 는 GridBattleController 가 fate_data 에 emit 하는 기존 field (`_fate_win_within_turns`, grid_battle_controller.gd:3347+3362) — VICTORY outcome 시 (`VICTORY_REACH_TILE` 포함) `round_now if round_now > 0 else 1` 자동 set. `op: <=` 는 HiddenConditionEvaluator 의 `_eval_fate_threshold` 가 이미 지원 (cross-ref: hidden_condition_evaluator.gd).
...
### 3.3 fate_data emit shape — 기존 substrate 정합 (검증 완료)

`win_within_turns` 는 fate_data 에 **이미 emit 됨** (grid_battle_controller.gd:3362). VICTORY outcome 매칭 (line 3340-3343, `VICTORY_ANNIHILATION` / `VICTORY_SURVIVE` / `VICTORY_REACH_TILE` / `VICTORY_ESCORT` 모두 포함) 시 line 3347 의 `_fate_win_within_turns = round_now if round_now > 0 else 1` 자동 set. ch08 의 REACH_TILE WIN 시 substrate 가 정확히 동작 — 신규 wiring 0 / 1-line add 0. impl 단계 의 첫 commit (data) 가 바로 chapter JSON edit 으로 진행 가능.

## 4. New Rules / Values

### 4.1 win_within_turns threshold = 6
...
- 신규 ChapterDefinition field — fate_data emit shape 가 win_within_turns 이미 보장 시 0 schema 변경.
...
| 3 | `test: ch08 alliance e2e — ★ trigger routing + canonical contrast + LOSS` | integration test 3 (★ at win_within_turns=6 / canonical at 7 / LOSS regression). `verify` step 은 §3.3 검증 완료로 skip — substrate 이미 정합. | commits 1-2 |
...
- [ ] **AC-2**: ch08 chapter JSON 의 `hidden_condition` = `{type: fate_threshold, field: win_within_turns, op: <=, value: 6}`.
...
- [ ] **AC-6**: GridBattleController fate_data 에 `win_within_turns` field emit — **이미 정합** (grid_battle_controller.gd:3362, VICTORY outcome 자동 set). 검증 sentinel test 추가.
...
- [ ] **AC-8**: Integration test — fixture 에서 win_within_turns=6 + WIN outcome + REACH_TILE 도달 시 DestinyBranchJudge resolves to `WIN_xiakou_united_advance`. win_within_turns=7 시 `WIN_xiakou_breakthrough` (canonical).
...
1. `test_ch08_hidden_condition_win_within_turns_le_6` — exact shape `{type: fate_threshold, field: win_within_turns, op: <=, value: 6}`.
...
5. `test_ch08_alliance_timing_triggers_hidden_branch_at_turn_6` — fate_data `{win_within_turns: 6}` + WIN outcome → DefaultDestinyBranchJudge.resolve → `WIN_xiakou_united_advance`.
6. `test_ch08_alliance_timing_falls_back_to_canonical_at_turn_7` — fate_data `{win_within_turns: 7}` + WIN → `WIN_xiakou_breakthrough` (default WIN branch).
...
|----|----------|-----------|-----------|
| OQ-1 | win_within_turns threshold = 6 vs 5 vs 7? | **6** (Plan §4.2 default, mid-range 합리) — Open for manual playtest tuning | Commit 4 후 manual playtest |
...
| OQ-3 | fate_data 에 `win_within_turns` field substrate 검증 | **검증 완료** — grid_battle_controller.gd:3347/3362 의 기존 wiring (VICTORY outcome auto-set). 본 OQ 는 closed (spec correction 시점). | Spec 확정 시 — closed |
...
| OQ-5 | `formation_turns` field 의 향후 처리 — chapter JSON 에서 *완전 제거* vs *legacy reference* 보존? | **ch08 만 교체** — `_fate_formation_turns` 는 substrate 에 존재 (grid_battle_controller.gd:3355) + 다른 chapter 가 향후 사용 가능성. ch08 의 hidden_condition 만 win_within_turns 로 교체, formation_turns field 자체는 보존. | Commit 2 시점 |
...
| OQ-8 | (Post-authoring correction) `turn_count` (hypothetical name) → `win_within_turns` (actual fate_data field) 발견 codification | **Codified**: spec authoring 시 fate_data emit shape 의 *정확한 field name* 사전 검증 필수. `grep -n '"\\(field_name\\)":' src/feature/grid_battle/grid_battle_controller.gd` 가 substrate 정합 검증의 standard step. 본 case 는 first authoring 의 `turn_count` 가 hypothetical 이라 actual `win_within_turns` 로 정정. ch05 §8 OQ-9 (Pillar lock 검토) 와 same family — spec authoring rule strengthening candidate. | Spec correction 시점 — closed |

---

> **Authoring note**: per `.claude/rules/design-docs.md` and S78/S79 codified rule (incremental authoring batch — core 2-3 decisions 잠근 후 spec 작성), 본 spec 은 핵심 3 결정 (hidden_condition = win_within_turns <=6 / victory_conditions REACH_TILE 유지 / echo_threshold 1 유지) 이 spec authoring 전 잠금되어 single-batch authoring. ADR 신규 불필요. Plan §4.2 의 "적 격퇴" hypothesis 는 narrative 정합 (도주→재집결) 위해 "REACH_TILE 도달" 로 변환 — 본 alignment 가 spec 의 decisive 결정.

...
> **UI spec**: `design/ux/battle-hud.md` (owns Visual/Audio, UI-GB-01..12, forecast contract)
>
...
- Volitional WAIT and CR-3 timeout/CR-3a invalid/CR-3b flush synthetic-WAIT all emit the identical `unit_waited(unit_id)` signal — AC-GB-21 consumers treat the signal as equivalent. The `acted_this_turn` mark, however, differs per the bookkeeping rule above: volitional → `false` (Ambush-bait), AI-failure substitution → `true` (Ambush-immune). Implementation sprint: AC-GB-25 should assert this divergence on flushed units.
...
Strategist's **distinguishing UI affordance** (vs. Commander's Rally combat mechanic; see unit-role.md CR-2 for Rally's +5% ATK ±15% cap values).
...
| CLEANUP | Result dismissed | Release map resources, clear unit instances, reset `cooldown_map`, reset `ai_soft_lock_counter = 0`, reset `movement_budget_map` | Emit `battle_complete(outcome)` to caller |
...
var hit_result := damage_calc.resolve(
    ...
    modifiers = {
        ...
        rng          = pinned_max_roll_rng,  # Counter-path contract: RNG pinned so the defender's evasion roll resolves to HIT AND damage_resolve's internal quantization favours the defender (i.e., this produces the worst-case counter damage against the original attacker, per §"Why max-roll" below). The same symbol is used at primary-attack forecast call sites, but with opposite semantics: primary-path callers pin the RNG to the ATTACKER-favourable extreme (best-case damage dealt). Callers MUST instantiate the two pinnings separately — the shared name is a naming convenience, not a shared state.
    },
)
...
| Battle HUD UX spec | `design/ux/battle-hud.md` (v1.1, v5.0 refactor — strict UI-only) | Output/Contract | UI-GB-01..12 contract (UI-GB-12 TacticalRead Extended Range added v5.0), forecast contract, chevron tier formulas, mobile DEFEND two-tap flow |
...
| Scenario Progression | Approved (pending /design-review) — `design/gdd/scenario-progression.md` | Input/Output | Battle payload (`map_id`, `unit_roster[]`, `deployment_positions{}`, `victory_conditions{}`, optional `battle_start_effects[]`) → `battle_complete(outcome_data)` signal relayed via GameBus autoload |
...
**AC-GB-07b (new v4.0; arithmetic corrected v5.0 pass-9a for BASE_CEILING=83)**: "Boundary-value damage tests: (a) ATK=30, DEF=99, D_mult=1.0, T_def=0 → MIN_DAMAGE floor fires (base = max(1, 30-99) = 1; result = 1). (b) ATK=200, DEF=15, D_mult=1.80, T_def=0 → BASE_CEILING clamp fires (base 185 → 83 via BASE_CEILING, then × 1.80 = 149.4 floored to 149; below DAMAGE_CEILING=180). (c) ATK=200, DEF=15, D_mult=1.0, T_def=0 → BASE_CEILING fires (base 185 → 83, then × 1.0 = 83). (d) ATK=100, DEF=0, D_mult=1.0, T_def=30 → BASE_CEILING + defense reduction max (base 100 → 83 via BASE_CEILING, then × 1.0 × (1 − 30/100) = 83 × 0.70 = 58.1 floored to 58). All four pass the floor/ceiling contract; results deterministic across runs."
— Type: Unit — Gate: BLOCKING
...
— Type: Integration — Mock: `tests/fixtures/mocks/animation_controller_stub.gd` — Fixture: constructor override parameter (no file) — File: `tests/integration/grid_battle/grid_battle_animation_watchdog_test.gd` — Gate: BLOCKING
...
— Type: Integration — Requires: mocked HP/Status `apply_status` + `apply_damage` for assertions (4)(5)(6) (status-write + counter-suppression assertions need the HP/Status pipeline in the loop, not just Grid Battle state) — Fixture: `tests/fixtures/grid_battle/defend_basic.yaml` — File: `tests/integration/grid_battle/grid_battle_defend_test.gd` (split pass-9a from `grid_battle_combat_flow_test.gd` because the shared file hosts Unit-type ACs 09/21/21b/23; AC-GB-21c Integration setup warranted its own file) — Gate: BLOCKING
...
- **BLOCKING (29 criteria):** AC-GB-01 through AC-GB-25 (25 base) + AC-GB-07b, AC-GB-10b, AC-GB-21b, AC-GB-21c (4 v4.0→v5.0 rewrites). Case-row additions (AC-GB-09 row h/i) counted under parent AC-GB-09.
- **ADVISORY (0 criteria).**
...
| `tests/integration/grid_battle/grid_battle_defend_test.gd` | AC-GB-21c (v5.0 — reclassified Integration pass-9a; assertions (4)(5)(6) need mocked HP/Status apply_status + apply_damage and counter-suppression scenario) |

...
| Battle HUD UX spec | `design/ux/battle-hud.md` (v1.1, v5.0 refactor — strict UI-only) | Output/Contract | UI-GB-01..12 contract (UI-GB-12 TacticalRead Extended Range added v5.0), forecast contract, chevron tier formulas, mobile DEFEND two-tap flow. **Downstream obligation (pass-11a.1 — game-designer R-6)**: battle-hud.md forecast contract must surface the "why Ambush fired" reason when Ambush suppresses a counter — since `show_wait_for_all_classes = false` hides WAIT from 5/6 classes, players cannot observe the `acted_this_turn = false` state that gates Ambush. Without an in-forecast reason, Ambush reads as random for most players. To be addressed in battle-hud.md next revision. |
...
(9) **`acted_this_turn` bookkeeping verification (pass-11a.1 — qa-lead F-1 + ai-programmer R-2 convergent)**: Per the CR-10 volitional-vs-AI-failure bookkeeping rule, flushed AI units (ids 3, 4) MUST set `acted_this_turn = true` (Ambush-immune — AI failure is not tactical choice). Assert: `acted_this_turn[3] == true AND acted_this_turn[4] == true` after flush completes. **Parameterised control case** — cross-reference AC-GB-21 fixture `wait_scout.yaml` (Scout volitional WAIT): in the same AC-GB-25 test scope, verify `acted_this_turn[scout_id] == false` post-volitional-WAIT to confirm the divergence between the two code paths. This assertion formalises the CR-10 doc note (which is now removed in favour of this AC gate — prevents the invariant from drifting silent during implementation)."
...
- **BLOCKING (29 criteria):** AC-GB-01 through AC-GB-25 (25 base) + AC-GB-07b, AC-GB-10b, AC-GB-21b, AC-GB-21c (4 v4.0→v5.0 rewrites). Case-row additions (AC-GB-09 row h/i) counted under parent AC-GB-09.
  - **Sub-classification (pass-11a)**: **23 fixture-authored or fixture-free** (runnable in current sprint) + **6 deferred-fixture** (cannot run until `.tres` or `skill_counter_*.yaml` files authored): AC-GB-01, AC-GB-11, AC-GB-15, AC-GB-23, AC-GB-25 (.tres dependencies), plus AC-GB-15 (skill_counter fixtures). Implementation sprint lead must prioritize fixture authoring before picking up these stories.
- **ADVISORY (0 criteria).**

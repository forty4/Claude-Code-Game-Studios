...
4. **Stacking**: Multiple Commanders each contribute +5% per Commander adjacent to the AFFECTED unit (not relative to each other). The contributions are summed additively, then capped at +15% total regardless of how many Commanders are adjacent. Formula: `rally_bonus = min(0.15, N_adjacent_alive_commanders × 0.05)` where `N_adjacent_alive_commanders` is the count of alive Commander units within Manhattan distance ≤ 1 of the affected unit. Three Commanders adjacent = 15% (cap). Four Commanders = still 15%. A Commander at distance 2 does not contribute even if adjacent to another Commander. (Mirrors `design/gdd/unit-role.md` EC-12.)
...
7. **AI awareness**: AI `get_battle_state_snapshot()` includes the per-unit `rally_bonus_active: float` field (0.0 to 0.15) reflecting the current Rally contribution to that unit's effective ATK. AI positioning logic may consider Commander proximity as a formation incentive.
...
**Purpose (Pillar 3)**: Commander is the force-multiplier class. Rally makes the Commander a high-value assassination target — killing the Commander collapses the formation's damage output, creating a tactical imperative to protect or eliminate it. The 3-Commander cap (15%) prevents absurd stacking in hypothetical many-Commander compositions while preserving the incentive for intelligent Commander positioning.
...
(b) **Two Commanders adjacent**: Two Commanders at (0,0) and (2,0), ally attacker at (1,0) (distance 1 from both). Assert `is_equal_approx(get_rally_bonus(attacker_id), 0.10)`.
(c) **Cap enforcement**: Three or more Commanders all adjacent to same ally. Assert `is_equal_approx(get_rally_bonus(attacker_id), 0.15)` regardless of N (cap hard floor).

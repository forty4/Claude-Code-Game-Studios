...
- **AC-SE-14**: Given chapter-1 mvp_shu.json with 2 beat_8_revelations entries (canonical_win + canonical_loss), when DestinyBranchChoice with `branch_key="WIN_changbanpo_default"` invoked, then F-SE-2 returns Dictionary with `text_key="ch01.beat8.win_changbanpo_default" AND cue_tag=&"tag.beat8.canonical_win"`.

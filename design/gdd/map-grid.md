...
- **Formation Bonus self-caching** *(v1.1 — per `design/gdd/formation-bonus.md` F-FB-1)*: Formation Bonus System does NOT call a `get_unit_at()` API on Map/Grid. It builds a local `coord_to_unit_id: Dictionary[Vector2i, int]` from the `units: Array[UnitState]` supplied at `round_started` invocation. Map/Grid need not expose a `get_unit_at()` API for Formation Bonus purposes. The only Map/Grid contract Formation Bonus holds is `get_adjacent_units(coord, faction) → Array[int]` (CR-FB-2 relationship adjacency). Any future consumer requiring point-lookup-by-coord should document its own caching contract rather than adding a Map/Grid API for it.

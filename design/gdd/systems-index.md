...
| 6 | Scenario Progression System (시나리오 진행) | Narrative | MVP | Feature | MAJOR REVISION NEEDED (v2.0 reviewed 2026-04-18 — pillar-integrity regression on Echo-gate; 34 blockers across 7 specialists) | design/gdd/scenario-progression.md | Save/Load (provisional), Balance/Data, Grid Battle, Hero Database, HP/Status, Destiny Branch (provisional), Destiny State (provisional), Story Event (provisional) |
...
| 11 | Damage/Combat Calculation (데미지/전투 계산) | Gameplay | MVP | Feature | In Review (rev 2.6 — eighth-pass re-review pending; 10 seventh-pass blockers resolved 2026-04-19 in clean-session sweep; 2 user design decisions made up front per `AskUserQuestion` discipline; cross-file coordination atomic damage-calc.md ↔ unit-role.md; arithmetic verified pre-close; regression risk the lowest of any sweep to date per review log rev 2.6 entry) | design/gdd/damage-calc.md | Unit Roles, HP/Status, Terrain, Balance/Data |
| 12 | HP/Status System (HP/상태) | Gameplay | MVP | Core | Designed | design/gdd/hp-status.md | Hero DB |
| 13 | Turn Order/Action Management (턴 순서/행동 관리) | Gameplay | MVP | Core | Needs Revision | design/gdd/turn-order.md | Hero DB — battle_ended ownership moved to Grid Battle per ADR-0001 (single-owner rule) |

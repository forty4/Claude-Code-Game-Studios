...
| 11 | Damage/Combat Calculation (데미지/전투 계산) | Gameplay | MVP | Feature | In Review (rev 2.7 — F-DC-5 `passive_multiplier` extended to accept `rally_bonus` via `ResolveModifiers.rally_bonus: float` per Grid Battle pass-11c CR-15 named downstream obligation 2026-04-19; Rally applied as multiplicative factor `(1.0 + rally_bonus)` in P_mult composition with class-mutex guards mirroring Charge/Ambush patterns; counter guard preserves "passives never echo on counter-attack" contract. Prior: rev 2.6 — eighth-pass re-review pending; 10 seventh-pass blockers resolved 2026-04-19 in clean-session sweep; 2 user design decisions made up front per `AskUserQuestion` discipline; cross-file coordination atomic damage-calc.md ↔ unit-role.md; arithmetic verified pre-close; regression risk the lowest of any sweep to date per review log rev 2.6 entry) | design/gdd/damage-calc.md | Unit Roles, HP/Status, Terrain, Balance/Data |

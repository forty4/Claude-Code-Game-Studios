...
| Field | Value |
|---|---|
| Version | 0.5 (partial-update: catches up to delta #6 ADR-0005 + delta #7 ADR-0010 — Foundation 5/5 + Core 1/2 → 2/2 closed) |
| Last Updated | 2026-04-30 |
| Engine | Godot 4.6 (pinned 2026-04-16) |
| Language | GDScript |
| Review Mode | lean |
| GDDs Covered | 10 of 14 MVP (scan source: `design/gdd/`, 2026-04-18) |
| ADRs Referenced | **11 Accepted**: ADR-0001/0002/0003 (2026-04-18) · ADR-0004 (2026-04-20) · ADR-0008 (2026-04-25) · ADR-0006/0012 (2026-04-26) · ADR-0009 (2026-04-28) · ADR-0007 (2026-04-30) · **ADR-0005 (2026-04-30 delta #6)** · **ADR-0010 (2026-04-30 delta #7)** |
| Technical Director Sign-Off | 2026-04-30 — **APPROVED WITH CONDITIONS** (partial-update v0.5; full sign-off pending Phase 3 Data Flow + Phase 4 API Boundaries + Phase 7 Architecture Principles authoring + Blocker §1 Turn Order → AI inversion via ADR-0011 + ADR-0001 line 372 prose drift cleanup + ADR-0012 line 89/260/352-353 unit_id type advisory carried from delta #7) |
...
| 5 | ADR Audit | ✅ written (11 Accepted ADRs audited 2026-04-30; 0 cross-conflicts blocking; 1 cross-doc unit_id type advisory carried from delta #7) |
| 6 | Required ADRs | ✅ refreshed 2026-04-30 (Foundation 5/5; Core 1/2 → 2/2; net-new 2-6 before Pre-Prod gate) |
...
- ADR-0011 Turn Order signal ownership finalization + AI inversion (Blocker §1; soft-dep'd by ADR-0012; soft-dep'd by ADR-0010 for `unit_turn_started` consumer contract)
...
**Count estimate (refreshed 2026-04-30 v0.5):** 2–6 additional ADRs before Pre-Production → Production gate (down from 4–8 at v0.4; ADR-0005 + ADR-0010 landed via deltas #6 + #7 — Foundation 5/5 Complete + Core 1/2 → 2/2). Next critical-path ADR is ADR-0011 Turn Order (closes ADR-0012's `get_acted_this_turn` soft-dep + ADR-0010's `unit_turn_started` provisional + Blocker §1 Turn Order → AI signal-inversion contract).
...
**11 Accepted ADRs as of 2026-04-30**. Audit derived from `/architecture-review` chain (2026-04-18 baseline + 7 delta passes 2026-04-20 / -25 / -26 / -28 / -30 / -30b / -30c — all PASS verdicts) cross-referenced against `architecture-traceability.md` v0.8 + `tr-registry.yaml` v9. Pattern stable at **7 invocations** of fresh-session /architecture-review for single-ADR escalation (avg ~2.7 corrections per delta — delta #6 anomaly 6 corrections for HIGH-risk first-of-its-kind ADR-0005; delta #7 = 3 corrections for first stateful battle-scoped Node form).
...
| **ADR-0012** Damage Calc | ✅ LOW (randi_range/snappedf pinned via mandatory CI tests AC-DC-49 + AC-DC-50; no post-cutoff API) | 4.6 | ✅ TR-damage-calc-001..013 (13/13); 53 GDD ACs map to ADR-0012 §GDD Requirements Addressed via 11-category coverage matrix | 2 wording corrections applied (AF-1 + Item 3); 1 advisory carried (AF-3 — non-blocking); ⚠️ **3 cross-doc precision-gap advisories carried for next ADR-0012 amendment**: (a) lines 89/260/352-353 `unit_id: StringName` → `unit_id: int` per ADR-0010 lock to ADR-0001 line 155 signal-contract source-of-truth (delta #7 Phase 4 Pair); (b) inherited delta #6 Item 10a (ADR-0001 line 168 String → StringName); (c) inherited delta #5 ADR-0007 carry (ADR-0001 line 372 prose drift) | ✅ Accepted 2026-04-26 |
...
Refreshed 2026-04-30 v0.5 against `architecture-traceability.md` v0.8 coverage summary. **Foundation layer at 5/5 Complete** (ADR-0005 Accepted via delta #6); **Core at 2/2** with HP/Status closed (ADR-0010 Accepted via delta #7; ADR-0011 Turn Order still gap — last Core-layer remaining); Feature at **1/3** (Damage Calc Accepted; AI / Grid Battle / Destiny Branch / Formation Bonus pending); Presentation at **0/1** (dual-focus pattern); Polish at **0/1** (Accessibility, gated on tier commit — already resolved 2026-04-18).
...
- **ADR-0011 Turn Order** — Core gap (last remaining); signal ownership finalization (`round_started`, `unit_turn_started`, `unit_turn_ended`); resolves **Blocker §1** (Turn Order → AI direct-call invariant violation per `turn-order.md:442` — must invert to signal-driven `ai_action_decided` contract). **Soft-dep'd by ADR-0012** §Dependencies (`get_acted_this_turn` interface) AND by ADR-0010 (`unit_turn_started` consumer contract for DoT tick + duration decrement + DEFEND_STANCE/DEMORALIZED expiry checks).
...
**2–6 ADRs** required before gate (down from 4–8 at v0.4; ADR-0005 + ADR-0010 landed via deltas #6 + #7 — total 7 ADRs landed 2026-04-20 through 2026-04-30):

- **Mandatory** (1): ADR-0011 Turn Order (last Core-layer gap; closes ADR-0010's `unit_turn_started` provisional + ADR-0012's `get_acted_this_turn` soft-dep + Blocker §1)
...
Per traceability v0.8: Foundation layer **5/5 Complete** (ADR-0005 Accepted via delta #6); Core layer **2/2** with HP/Status closed (ADR-0010 Accepted via delta #7). All Foundation + Core epics now eligible for `/create-epics` except hp-status (immediate next candidate post-Acceptance) + turn-order (gated on ADR-0011). Critical path is now ADR-0011 → first Vertical-Slice candidate epics (Grid Battle + AI + Battle HUD + Battle Preparation).

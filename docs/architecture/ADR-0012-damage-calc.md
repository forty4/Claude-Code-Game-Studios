...
| Field | Value |
|-------|-------|
| **Depends On** | ADR-0001 (GameBus, Accepted 2026-04-18) — Damage Calc is on the **non-emitter list**; ResolveResult is a pure return value, never a signal payload. ADR-0008 (Terrain Effect, Accepted 2026-04-25) — `terrain.get_combat_modifiers(atk, def) -> CombatModifiers` returning already-clamped `terrain_def ∈ [-30, +30]` and `terrain_evasion ∈ [0, 30]`; the cap constants `MAX_DEFENSE_REDUCTION = 30` and `MAX_EVASION = 30` are owned by ADR-0008, not this ADR. **ADR-0006 (Balance/Data, NOT YET WRITTEN — soft / provisional)**: `DataRegistry.get_const(key)` for 11 constants (9 consumed + 2 owned). Workaround until ADR-0006 lands: ship with direct constant reads from `assets/data/balance/entities.yaml` via a thin `BalanceConstants` wrapper; migrate to ADR-0006's pipeline call when Accepted. The migration is API-stable (call sites unchanged); only the wrapper's internal load mechanism switches. **ADR-0009 Unit Role (NOT YET WRITTEN — soft / provisional)**: CLASS_DIRECTION_MULT[4][3] table + BASE_DIRECTION_MULT[3] vector, owned by Unit Role; Damage Calc consumes via Unit Role's read-only constant accessors. Workaround: ADR-0012 commits to the table values as locked by `unit-role.md` §EC-7; ADR-0009 will *ratify* (not negotiate) those values when written. **ADR-0010 HP/Status (NOT YET WRITTEN — soft / provisional)**: `hp_status.get_modified_stat(unit_id: StringName, stat_name: String) -> int` interface returning effective stats with all buffs/debuffs and DEFEND_STANCE penalty pre-folded; MIN_DAMAGE ≥ 1 contract per `hp-status.md:98`. **ADR-0011 Turn Order (NOT YET WRITTEN — soft / provisional)**: `turn_order.get_acted_this_turn(unit_id: StringName) -> bool` for the Scout Ambush gate. |
...
| **Ordering Note** | Provisional-dependency strategy mirrors **ADR-0008 → ADR-0006 precedent** (ADR-0008 was Accepted 2026-04-25 with a soft-depend on the not-yet-written ADR-0006 Balance/Data; the workaround was direct `FileAccess` + `JSON.parse_string` config loading with a stable migration path when ADR-0006 lands). The same pattern applies here for ADR-0006 + 0009 + 0010 + 0011: ADR-0012 commits to the interface signatures **verbatim from the GDDs** that those four ADRs will eventually ratify. The interface contracts are locked by the GDDs (which are all APPROVED for the relevant sections); ADRs 0006/0009/0010/0011 will ratify rather than negotiate. The risk is bounded: each upstream ADR can only widen, never narrow, the ADR-0012-locked interface (a narrowing change would force a reciprocal ADR-0012 amendment, caught by `/architecture-review` cross-conflict detection). |
...
- `get_modified_stat(unit_id: StringName, stat_name: String) -> int` returns effective stat with all buffs/debuffs and DEFEND_STANCE penalty pre-folded.
...
- CLASS_DIRECTION_MULT[4 classes][3 directions] table — Cavalry/Scout/Infantry/Archer × FRONT/FLANK/REAR. Cavalry REAR = 1.09 (rev 2.8, was 1.20 pre-Rally-cap-fix).
...
- `get_acted_this_turn(unit_id: StringName) -> bool` for Scout Ambush gate.
...
|---|---|---|---|
| **HP/Status** | `hp_status.get_modified_stat(unit_id: StringName, stat_name: String) -> int` | `hp-status.md:508`, §F-1 | ADR-0010 (NOT YET WRITTEN — provisional) |
...
| **Unit Role** | Constants: `UnitRole.BASE_DIRECTION_MULT[3]`, `UnitRole.CLASS_DIRECTION_MULT[4][3]`. Both compile-time tables. | `unit-role.md` §EC-7, lines 180–193, 503–506 | ADR-0009 (NOT YET WRITTEN — provisional) |
| **Turn Order** | `turn_order.get_acted_this_turn(unit_id: StringName) -> bool` | `turn-order.md` lines 397, 401, 407–408 | ADR-0011 (NOT YET WRITTEN — provisional) |
| **Balance/Data** | `DataRegistry.get_const(key: String) -> Variant` (typed by caller) | `balance-data.md` lines 206, 334; `entities.yaml` | ADR-0006 (NOT YET WRITTEN — provisional) |
...
```
        ...
        │     │           │   UnitRole.CLASS_DIRECTION_MULT[4][3]                   │
...
- **Provisional-dependency surface area** — 4 future ADRs (ADR-0006/0009/0010/0011) all have to ratify their interfaces verbatim or trigger a reciprocal ADR-0012 amendment. Mitigation: `/architecture-review` cross-conflict detection runs every time a new ADR is Accepted; the registry's `referenced_by` field surfaces ADR-0012 in each upstream ADR's review.
...
- **ADR-0006 Balance/Data (NOT YET WRITTEN — Sprint 1 S1-09 Nice-to-Have)** — when Accepted, ratifies `DataRegistry.get_const(key)` interface for the 11 constants. Reciprocal-update obligation: ADR-0006's `referenced_by` field includes ADR-0012.
- **ADR-0009 Unit Role (NOT YET WRITTEN — Sprint 1 S1-06 Should-Have)** — when Accepted, ratifies `UnitRole.BASE_DIRECTION_MULT[3]` and `UnitRole.CLASS_DIRECTION_MULT[4][3]` table interfaces. Reciprocal-update obligation symmetric.
- **ADR-0010 HP/Status (NOT YET WRITTEN — post-Sprint-1 priority)** — when Accepted, ratifies `hp_status.get_modified_stat(unit_id, stat_name)` interface. Reciprocal-update obligation symmetric.
- **ADR-0011 Turn Order (NOT YET WRITTEN — post-Sprint-1 priority)** — when Accepted, ratifies `turn_order.get_acted_this_turn(unit_id)` interface. Reciprocal-update obligation symmetric.
...
This ADR explicitly soft-depends on 4 not-yet-written ADRs (0006, 0009, 0010, 0011). The strategy mirrors ADR-0008 → ADR-0006 precedent (proven: ADR-0008 was Accepted 2026-04-25 with the same pattern; the workaround direct-loading code shipped cleanly and is migration-stable for the future ADR-0006). Audit trail:
...
- When ADR-0006 / 0009 / 0010 / 0011 are authored, each `/architecture-review` of the new ADR runs cross-conflict detection against ADR-0012's locked interfaces. A narrowing change in any upstream ADR triggers a reciprocal ADR-0012 amendment.

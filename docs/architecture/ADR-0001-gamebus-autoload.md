...
| Signal | Payload Type | Payload Fields | Emitter | Subscribers (MVP) | Frequency |
|---|---|---|---|---|---|
| `destiny_branch_chosen` | `DestinyBranchChoice` (Resource) | RATIFIED via /architecture-review delta #13 (2026-05-04) — 9-field shape per ADR-0018 §Decision §Payload Form: `chapter_id: String`, `branch_key: String`, `outcome: BattleOutcome.Result`, `echo_count: int`, `is_draw_fallback: bool`, `is_canonical_history: bool`, `reserved_color_treatment: bool`, `is_invalid: bool`, `invalid_reason: StringName`. Construction via `DestinyBranchChoice.new()` + 9 field assignments OR via `DestinyBranchChoice.invalid(reason: StringName)` factory (12-entry F-DB-3 vocabulary). Supersedes the prior provisional 5-field shape (`chapter_id`/`branch_key`/`revelation_cue_id`/`required_flags`/`authored`) which is removed — `revelation_cue_id` and `required_flags` and `authored` no longer exist. | DestinyBranchJudge (constructed transient at beat 7 tap-exit per ADR-0018 — RefCounted scope drop after `resolve()` returns; ScenarioRunner emits per CR-DB-4 emission ownership) | ScenarioRunner (consumes in BRANCH_JUDGMENT), Story Event (beat 8 revelation content keys on `is_canonical_history` for Pillar 4 contrast), Destiny State (consumes `is_draw_fallback` + `echo_count` for Marked Hand register), Save/Load (persists `branch_key` + `is_canonical_history` per ADR-0003 SaveContext schema) | discrete |
...
> **Pillar 2 mechanical expression**: `reserved_color_treatment` triggers V-DB-1 reserved-color reveal at Beat 7 (ratified 2026-05-04 via ADR-0018 §F-DB-2 derivation rule). **Pillar 4 mechanical expression**: `is_canonical_history` keys Beat 8 contrast (Story Event #10 VS — canonical 演義 vs rewritten history payload-level enforcement). **Invalid-path emission contract**: `is_invalid == true` payloads ARE emitted (preserves AC-SP-17 exactly-one-emission per chapter); subscribers MUST gate content reads on `is_invalid == false` per BLOCKING per-field contract (destiny-branch §Bidirectional rev 1.2 D1). The 12-entry `invariant_violation:*` `StringName` vocabulary lives in destiny-branch GDD §F-DB-3.

> **Cross-doc constraint (BattleOutcome top-level class_name)**: `BattleOutcome` MUST be declared as a top-level `class_name BattleOutcome` in its own script file (NOT as inner class of another script) to satisfy `DestinyBranchChoice.@export var outcome: BattleOutcome.Result = BattleOutcome.Result.LOSS` parse-time resolution per Godot 4.6 global class registry. Inner-class references are NOT resolvable at parse time and would silently break `ResourceLoader.load()` for any saved `DestinyBranchChoice`. SATISFIED in shipped code via `src/core/payloads/battle_outcome.gd:10` (top-level `class_name BattleOutcome extends Resource`). Cross-doc constraint coordinated with grid-battle v5.0 GDD revision (ADR-0018 §Migration Plan §6).
...
| `save_loaded` | `SaveContext` (Resource) | `ctx: SaveContext` — emitted by SaveManager after `SaveMigrationRegistry.migrate_to_current(ctx)` succeeds; carries the migrated payload to subscribers for rehydration. CR-SL-19/20 idempotency contract — handler must be re-runnable on the same ctx without state divergence. CR-SL-22 never-crash invariant — failure path emits `save_load_failed` instead of `save_loaded`. | SaveManager (after migrate_to_current succeeds in `load_latest_checkpoint`) | Destiny State (rehydrates `_full_archive` + `_chapter_echo_counts` + `_flags_to_set` from payload via CONNECT_DEFERRED handler) | discrete |
...
#### 11. Story Event #10 domain (Emitter: StoryEvent autoload)

| Signal | Payload Type | Payload Fields | Emitter | Subscribers (MVP) | Frequency |
|---|---|---|---|---|---|
| `story_event_resolved` | `int, StringName, String, StringName` | `beat_number: int`, `variant_key: StringName` (one of 6: `canonical_win` / `rewritten_win` / `draw_partial` / `draw_echo_marked` / `draw_fallback` / `defeat`), `text_key: String` (chapter authoring lookup), `cue_tag: StringName` (cinematic-layer hook reserved for sprint-9+ Story Event cinematic-layer authoring; sentinel `&""` at MVP) | StoryEvent autoload (Beat 8 revelation lookup + Beat 9 outro emission per S7-06 GDD CR-SE-2 + S8-09 implementation) | Story Event UI (text rendering + cinematic-layer cue dispatch in sprint-9+) | discrete |
| `story_event_invalid_path_detected` | `StringName, String` | `reason: StringName` (12-entry F-DB-3 invariant_violation:* vocabulary), `choice_chapter_id: String` (chapter where the invalid path was detected) | StoryEvent autoload (D1 BLOCKING contract per CR-SE-12 — fires when DestinyBranchChoice.is_invalid==true; pre-empts Beat 8 revelation emission) | Invalid-Path UI (sprint-9+ — error dialog with retry/abandon affordances) | discrete (rare — invariant violation only) |
| `story_event_revelation_committed` | `String, String, StringName` | `chapter_id: String`, `branch_key: String`, `register: StringName` (one of 2: `solemn` for canonical-non-Marked-Hand revelations, `marked` for Marked Hand register revelations per CR-SE-12 dual-register contract) | StoryEvent autoload (Beat 8 revelation telemetry tagging — fires AFTER story_event_resolved) | Telemetry / analytics (sprint-9+ — register tagging for Pillar 2/4 sole-carrier discipline metrics) | discrete |

> **Domain note (ADR-0001 vs Domain 6 distinction)**: Domain 11 "Story Event #10" is intentionally separate from Domain 6 "Story Event / Beat presentation (BeatConductor)". The two domains correspond to different emitters with non-overlapping responsibilities: Domain 6's BeatConductor signals are the cinematic-layer cue channel (visual + audio cue fan-out, beat sequence completion gates) — still PROVISIONAL pending sprint-9+ Story Event cinematic-layer authoring. Domain 11's StoryEvent autoload signals are the narrative-substrate channel (variant-keyed text resolution + invalid-path UI carve-out + revelation telemetry) — ratified-at-source via S8-09 implementation per S7-06 GDD CR-SE-2 + S8-11 chapter-1 e2e integration validation. The two domains will eventually orchestrate: StoryEvent emits story_event_resolved → Story Event UI consumer dispatches a BeatConductor-owned cue (sprint-9+ cinematic layer) → BeatConductor emits beat_visual_cue_fired + beat_audio_cue_fired → Story Event UI renders. The domain split keeps the narrative-substrate emit channel (Domain 11) decoupled from the cinematic-cue emit channel (Domain 6) so that sprint-9+ cinematic-layer authoring can ratify Domain 6 PROVISIONAL signals without touching Domain 11 ratified-at-source signals.

**Total signal count: 31 signals across 11 domains** (post-save-load story-002 minor amendment 2026-05-08 — added `save_loaded(SaveContext)` to Persistence domain #9 as 4th signal; was 30 pre-amendment).

**PROVISIONAL signals: 2** (all payload-shape-provisional; all emitter+name locked):
...
**PROVISIONAL → RATIFIED (2026-05-04 via /architecture-review delta #13)**:
- `destiny_branch_chosen` — shape ratified by ADR-0018 as `(choice: DestinyBranchChoice)` using the **9-field DestinyBranchChoice typed Resource** (chapter_id / branch_key / outcome / echo_count / is_draw_fallback / is_canonical_history / reserved_color_treatment / is_invalid / invalid_reason). Supersedes the provisional 5-field shape (`chapter_id`/`branch_key`/`revelation_cue_id`/`required_flags`/`authored`) — `revelation_cue_id` + `required_flags` + `authored` fields no longer exist. New shape carries `is_canonical_history` for Pillar 4 contrast (Beat 8 Story Event #10 VS) + `is_invalid` for invalid-path emission contract (preserves AC-SP-17 exactly-one-emission). Evolution Rule #4 minor amendment, NOT supersession. Same-patch updated via this delta. Cross-doc constraint: `BattleOutcome` MUST be top-level `class_name` (SATISFIED in shipped code via `src/core/payloads/battle_outcome.gd:10`).
...
| 2026-05-04 | Minor amendment (§Evolution Rule #4): ratified `scenario_beat_retried` PROVISIONAL → RATIFIED with shipped 3-field EchoMark schema; widened `scenario_complete` payload String → ScenarioResult Resource per ADR-0017 §CR-16 + F-SP-4 (cross-ADR conflict #1 resolved via Path A user decision). PROVISIONAL signal count 4 → 3. Required by ADR-0017 Scenario Progression acceptance. No supersession. | /architecture-review delta #12 |
| 2026-05-04 | Minor amendment (§Evolution Rule #4): ratified `destiny_branch_chosen` PROVISIONAL 5-field → RATIFIED 9-field shape per ADR-0018 §Decision §Payload Form (DestinyBranchChoice 9-field typed Resource). Removed `revelation_cue_id` + `required_flags` + `authored` fields; added `outcome: BattleOutcome.Result` + `echo_count: int` + `is_draw_fallback: bool` + `is_canonical_history: bool` (Pillar 4) + `reserved_color_treatment: bool` (Pillar 2 derivation) + `is_invalid: bool` + `invalid_reason: StringName` (12-entry F-DB-3 vocabulary). PROVISIONAL signal count 3 → 2. Cross-doc constraint added: `BattleOutcome` top-level `class_name` requirement (SATISFIED in shipped `src/core/payloads/battle_outcome.gd:10`). Pillar 2 + Pillar 4 mechanical-expression note rewritten. Required by ADR-0018 Destiny Branch acceptance. No supersession. | /architecture-review delta #13 |
| 2026-05-06 | Minor amendment (§Evolution Rule #1 + #4): added Domain 11 "Story Event #10 (emitter: StoryEvent autoload)" with 3 NEW signals — `story_event_resolved(beat_number: int, variant_key: StringName, text_key: String, cue_tag: StringName)` + `story_event_invalid_path_detected(reason: StringName, choice_chapter_id: String)` + `story_event_revelation_committed(chapter_id: String, branch_key: String, register: StringName)`. Signal count 27 → 30; domain count 10 → 11; PROVISIONAL count unchanged at 2. Signals already declared in source `src/core/game_bus.gd:62-69` per S8-09 sprint-8 implementation commit `6dbf494` 2026-05-05; this minor amendment closes the post-source-of-truth ratification loop (3rd-precedent post-source ratification pattern after delta #12 + #13). Required by S7-06 Story Event #10 GDD CR-SE-2/3/19 + S8-09 implementation + S8-11 chapter-1 e2e integration validation. Domain 6 "Story Event / Beat presentation (BeatConductor)" remains PROVISIONAL — Domain 11 is the narrative-substrate channel; Domain 6 is the cinematic-cue channel (intentional split). No supersession. | /architecture-review delta #15 |
| 2026-05-08 | Minor amendment (§Evolution Rule #1 + #4): added `save_loaded(ctx: SaveContext)` to Persistence domain (#9) — 4th signal in domain. Required by save-load story-002 cross-chapter continuity (OQ-SL-3 resolution: signal-driven dispatch over method-call). SaveManager emits after `migrate_to_current(ctx)` succeeds in `load_latest_checkpoint`; Destiny State subscribes via CONNECT_DEFERRED for idempotent rehydration per CR-SL-19/20 (handler re-runnable on same ctx without state divergence). CR-SL-22 never-crash invariant honored — failure path emits `save_load_failed` instead. Signal count 30 → 31; PROVISIONAL count unchanged at 2; domain count unchanged at 11. No supersession. | save-load story-002 /dev-story 2026-05-08 |

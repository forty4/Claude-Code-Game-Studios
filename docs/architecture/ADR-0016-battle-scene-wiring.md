...
- **R-3**: BattleScene._ready() executes a 6-step mount sequence in DI dependency order: (1) MapGrid + load from MapResource → add_child as first sibling under BattleScene root; (2) BattleCamera.setup(map_grid) → add_child; (3) HPStatusController + per-unit initialize_unit loop → add_child; (4) TurnOrderRunner + initialize_battle(roster) → add_child; (5) GridBattleController.setup(...) 8-param → add_child; **(5.5) AISystem.setup(grid_controller) → add_child** *(inserted via /architecture-review delta #14 2026-05-05 per ADR-0019 §Mount Order; battle-scoped Node 6th invocation; sprint-7+ S7-04 implementation patch adds the actual mount call site; Path A "5.5" insertion preserves existing 1-6 numbering — full 1-7 renumber deferred to sprint-7+ S7-02 same-patch as sprint-6 mock encoder deletion + lint phase-flip + main_scene revert)*; (6) BattleHUD.setup(...) 9-param → HUDLayer.add_child(hud) *(BattleHUD AI-thinking-indicator subscription per ADR-0019 §Migration Plan §7 requires AISystem present at HUD `_ready()` — step 5.5 insertion satisfies this ordering).

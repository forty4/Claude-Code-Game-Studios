...
| Source — architecture: | `docs/architecture/architecture.md` v0.6 (refreshed 2026-04-30 — Core 3/3 Complete this delta; mandatory ADR list pruned 1 → 0) |
| Source — TR registry: | `docs/architecture/tr-registry.yaml` v10 |
| Source — /architecture-review: | `docs/architecture/architecture-review-2026-04-18.md` (PASS) + `architecture-review-2026-04-20.md` (PASS delta, ADR-0004 accepted) + `architecture-review-2026-04-25.md` (PASS delta, ADR-0008 accepted) + `architecture-review-2026-04-26.md` (PASS delta, ADR-0012 accepted) + `architecture-review-2026-04-28.md` (PASS delta, ADR-0009 accepted) + `architecture-review-2026-04-30.md` (PASS delta, ADR-0007 accepted) + `architecture-review-2026-04-30b.md` (PASS delta, ADR-0005 accepted — Foundation 5/5 Complete) + `architecture-review-2026-04-30c.md` (PASS delta, ADR-0010 accepted — first Core-layer stateful battle-scoped Node ADR; closes ADR-0012 `get_modified_stat`/`apply_damage` upstream Core soft-dep) + **`architecture-review-2026-04-30d.md` (PASS delta, ADR-0011 accepted — last Core-layer ADR; closes ADR-0010 `unit_turn_started` provisional + ADR-0012 `get_acted_this_turn` soft-dep + Blocker §1 Turn Order → AI signal-inversion contract; first all-Foundation+Core-Complete state)** |
| GDDs scanned | 10 of 14 MVP (2026-04-18) + map-grid.md re-scan (2026-04-20) + terrain-effect.md re-scan (2026-04-25) + damage-calc.md re-scan (2026-04-26) + unit-role.md re-scan (2026-04-28) + hero-database.md re-scan (2026-04-30) + input-handling.md re-scan (2026-04-30 delta #6) + hp-status.md re-scan (2026-04-30 delta #7) + **turn-order.md re-scan (2026-04-30 delta #8)** |
| TRs extracted (this session) | 201 |
| TRs registered (permanent IDs) | 142 |
| ADR coverage: | **12 ADRs (all Accepted — Platform 3/3 + Foundation 5/5 + Core 3/3 + 1 Feature; first all-Foundation+Core-Complete state)** |
...
| Core | ~68 | **3** (ADR-0008 + ADR-0010 + **ADR-0011** Accepted) | 0 more | ✅ **3/3 Complete** (last Core-layer gap closed; first all-Foundation+Core-Complete state) |
...
**Net-new ADRs required before Pre-Production → Production gate**: 1–5 (ADR-0011 landed this pass; closes ADR-0010's `unit_turn_started` provisional + ADR-0012's `get_acted_this_turn` soft-dep + Blocker §1 Turn Order → AI signal-inversion contract per `architecture.md`; mandatory ADR list pruned 1 → 0). All Foundation + Core layers now complete; remaining ADRs are strongly-recommended-or-can-defer Vertical Slice candidates (AI / Grid Battle / Formation Bonus / UI dual-focus / Destiny Branch).

---

## Registered TR-to-ADR map (source: tr-registry.yaml v10)

These 142 requirements have permanent IDs and are already covered by an Accepted ADR.

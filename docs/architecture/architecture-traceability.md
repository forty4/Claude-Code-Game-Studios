...
| Source — architecture: | `docs/architecture/architecture.md` v0.4 (refreshed 2026-04-30 — Foundation 4/5 layer gap closing this delta) |
| Source — TR registry: | `docs/architecture/tr-registry.yaml` v8 |
| Source — /architecture-review: | `docs/architecture/architecture-review-2026-04-18.md` (PASS) + `architecture-review-2026-04-20.md` (PASS delta, ADR-0004 accepted) + `architecture-review-2026-04-25.md` (PASS delta, ADR-0008 accepted) + `architecture-review-2026-04-26.md` (PASS delta, ADR-0012 accepted) + `architecture-review-2026-04-28.md` (PASS delta, ADR-0009 accepted) + `architecture-review-2026-04-30.md` (PASS delta, ADR-0007 accepted) + **`architecture-review-2026-04-30b.md` (PASS delta, ADR-0005 accepted — Foundation 5/5 Complete)** |
| GDDs scanned | 10 of 14 MVP (2026-04-18) + map-grid.md re-scan (2026-04-20) + terrain-effect.md re-scan (2026-04-25) + damage-calc.md re-scan (2026-04-26) + unit-role.md re-scan (2026-04-28) + hero-database.md re-scan (2026-04-30) + **input-handling.md re-scan (2026-04-30 delta #6)** |
| TRs extracted (this session) | 163 |
| TRs registered (permanent IDs) | 104 |
| ADR coverage: | 10 ADRs (all Accepted — **5 Foundation Complete** + 1 Core + 1 Feature + 2 Foundation/Core-bridge for Unit Role + Foundation content-data layer for Hero DB + **first HIGH engine-risk ADR for Input Handling Foundation**) |
...
**Net-new ADRs required before Pre-Production → Production gate**: 3–7 (ADR-0005 landed this pass; closes Foundation layer entirely; first HIGH engine-risk ADR — addresses 4.6 dual-focus + 4.5 SDL3 + 4.5 recursive Control disable + 4.5 Android edge-to-edge in same ADR). Next critical path is Core layer: ADR-0010 HP/Status + ADR-0011 Turn Order, both soft-dep'd by ADR-0012 Damage Calc.

---

## Registered TR-to-ADR map (source: tr-registry.yaml v8)

These 104 requirements have permanent IDs and are already covered by an Accepted ADR.

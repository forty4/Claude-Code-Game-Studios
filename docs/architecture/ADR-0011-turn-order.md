...
**Same-patch amendment 2026-05-01 (story-004 — declare_action + token validation + DEFEND_STANCE locks + 5 ActionType enum)**: UnitTurnState extended from 9 → 10 fields with `defend_stance_active: bool` to support CR-4c MOVE-lock-after-DEFEND semantics. Set true by `declare_action(DEFEND)` in T5; reset to false at T4 by `_activate_unit_turn` (mirrors the existing token-flag reset list). Within-turn semantics (DEFEND-then-MOVE same turn → MOVE_LOCKED_BY_DEFEND_STANCE) is what story-004 implements; cross-turn HP/Status SE-3 persistence (CR-4b release timing on subsequent MOVE/ATTACK) is DEFERRED to story-005+ HP/Status integration which replaces this stub field with `HPStatusController.has_status_effect(unit_id, StatusEffect.DEFEND_STANCE)`. TR-turn-order-004 revised to 10 fields same-patch (`docs/architecture/tr-registry.yaml` `revised: "2026-05-01"`). Field count remains within RefCounted-wrapper-as-typed-data-carrier discipline; no functional change to TurnOrderSnapshot or TurnOrderEntry. The T4 reset path now writes 6 fields (move_token_spent + action_token_spent + accumulated_move_cost + acted_this_turn + turn_state + defend_stance_active) per the G-15 forward-look reset list earmarked for story-007 lint scaffold.

**Wrapper count amendment 2026-05-01 (story-004)**: §Decision wrapper count was originally "3 RefCounted typed wrappers" (UnitTurnState + TurnOrderSnapshot + TurnOrderEntry). Story-002 added `BattleUnit` (RefCounted stub per §Migration Plan §3 — soft-dep on Battle Preparation ADR). Story-004 adds `ActionResult` (RefCounted typed return for `declare_action()` per IN-2 option (a) — typed return matches ADR-0010 / ADR-0012 precedent over Dictionary-return regression) + `ActionTarget` (RefCounted stub per IN-2 — minimal 2-field placeholder; Battle Preparation ADR / Grid Battle epic will ratify the full target descriptor with terrain/range/LoS validation). Current wrapper count: **6** (UnitTurnState 10-field + TurnOrderSnapshot 2-field + TurnOrderEntry 5-field + BattleUnit 4-field stub + ActionResult 3-field + ActionTarget 2-field stub). The §State Ownership "3 wrappers" prose is superseded; treat the wrapper inventory in `src/core/` (under `class_name Action*` + `class_name BattleUnit` + `class_name TurnOrder*` + `class_name UnitTurnState`) as authoritative going forward.

...
Accepted (2026-05-06 via `/architecture-review` delta #15 — combined-session escalation pattern 5th invocation; **first cross-calendar-day fresh-session escalation** in the project. ADR-0020 Proposed 2026-05-05 sprint-8 S8-01 → Accepted 2026-05-06 via this delta after a /clear-bound fresh session. Pattern stable at 5 invocations across deltas #11 (ADR-0014/0017) + #12 (ADR-0017 widening) + #13 (ADR-0018) + #14 (ADR-0019) + #15 (this delta). All 4 net-new forbidden_patterns + 4 net-new TRs + 1 amended TR + 2 amended forbidden_pattern descriptions registered same-patch via delta #15. ADR-0001 minor amendment for 3 NEW Story Event #10 signals already in source bundled into the same delta per post-source-of-truth ratification convention (3rd-precedent after delta #12 + #13).)
...
| 2026-05-06 | Status: Proposed → **Accepted** via `/architecture-review` delta #15 (`docs/architecture/architecture-review-2026-05-06.md`). Verdict PASS (0 BLOCKING + 0 ADVISORY). All 4 net-new forbidden_patterns + 4 net-new TRs + 1 amended TR + 2 amended forbidden_pattern descriptions registered same-patch. ADR-0001 minor amendment bundled (3 NEW Story Event #10 signals — `story_event_resolved` + `story_event_invalid_path_detected` + `story_event_revelation_committed` — registered formally per Evolution Rule #1 + #4; signals already shipped in `src/core/game_bus.gd:62-69` per S8-09 commit `6dbf494`). ADR-0005 changelog backfilled ratifying field count 6 → 8 + 4 net-new forbidden_patterns. ADR-0014 changelog backfilled ratifying `is_tile_in_move_range` + `is_tile_in_attack_range` as InputRouter consumer surfaces. ADR-0015 changelog backfilled ratifying 4 InputRouter integration surfaces (TPP callee + 2 GameBus subscribers + undo `_handle_event` caller exception). Combined-session escalation pattern stable at 5 invocations; **first cross-calendar-day fresh-session escalation** in the project (Proposed 2026-05-05 → Accepted 2026-05-06; same-session-ban discipline satisfied via independent /clear-bound sessions). Sprint-8 Must-Have critical-path UNBLOCKED — S8-02..S8-07 input-handling chain ready for /dev-story execution. S8-01 sprint-status row closed in same patch (9th-streak in-patch sprint-status hygiene close per sprint-7 retro AI #1; pattern stability target 6+ MET). Total accepted ADR count: 19 → 20. tr-registry v15 → v16 (254 → 258 TRs). architecture.yaml v13 → v14. ADR-0001 signal count 27 → 30 across 11 domains; PROVISIONAL count unchanged at 2. |

...
| **Soft / Provisional** | (1) **Camera ADR (NOT YET WRITTEN — soft / provisional downstream)**: `Camera.screen_to_grid(screen_pos: Vector2) -> Vector2i` for touch/click hit-testing; Camera owns `camera_zoom_min = 0.70` enforcement per F-1; Camera subscribes to `camera_pan` / `camera_zoom_in/out` / `camera_snap_to_unit` signals. ADR-0005 commits to these signatures verbatim from `design/gdd/input-handling.md` §Bidirectional Contracts; downstream Camera ADR ratifies (does not negotiate). Mirrors 4-precedent provisional-dep pattern (ADR-0008→0006 / ADR-0012→0009/0010/0011 / ADR-0009→0007 / ADR-0007→Formation Bonus). (2) **Grid Battle ADR (NOT YET WRITTEN — soft / provisional downstream)**: `GridBattleController.is_tile_in_move_range(coord: Vector2i) -> bool` + `is_tile_in_attack_range(coord: Vector2i) -> bool` for state-transition validation. InputRouter NEVER computes ranges itself — it queries Grid Battle for every transition gate. Migration parameter-stable when Grid Battle ADR lands. (3) **Battle HUD ADR-0015 (Accepted 2026-05-03 via /architecture-review delta #10 — RATIFIED parameter-stable; no code change to ADR-0005 required)**: `BattleHUD.show_unit_info(unit_id: int) -> void` + `show_tile_info(coord: Vector2i) -> void` for Touch Tap Preview Protocol (CR-4a) ratified per ADR-0015 §4 Public API; `BattleHUD` reads `InputRouter.get_active_input_mode() -> InputMode` for hint icon updates per ADR-0015 §5 `_on_input_mode_changed` handler. ADR-0015 also subscribes to `input_state_changed` for S5 INPUT_BLOCKED `MOUSE_FILTER_IGNORE` recursive disable per ADR-0015 §5. (4) **Settings/Options ADR (NOT YET WRITTEN — soft / provisional downstream)**: `InputRouter.set_binding(action: StringName, event: InputEvent) -> void` for runtime key remapping per CR-1b. (5) **Tutorial ADR (NOT YET WRITTEN — soft / provisional downstream)**: subscribes to `input_action_fired` for tutorial step detection per GDD §Interactions. |
...
|---|---|---|
| Camera (ADR-0013 Accepted 2026-05-02) | `Camera.screen_to_grid(screen_pos: Vector2) -> Vector2i` | RATIFIED parameter-stable per ADR-0013 §10 — signature unchanged |
| Camera (ADR-0013 Accepted 2026-05-02) | Camera enforces `camera_zoom_min = 0.70` per F-1 | RATIFIED — constant value locked at this ADR; ADR-0013 honors via clamp |
| Grid Battle (ADR-0014 Accepted 2026-05-02) | `GridBattleController.is_tile_in_move_range(coord: Vector2i) -> bool` | RATIFIED parameter-stable per ADR-0014 §query_api (delta #10 backfill) |
| Grid Battle (ADR-0014 Accepted 2026-05-02) | `GridBattleController.is_tile_in_attack_range(coord: Vector2i) -> bool` | RATIFIED parameter-stable per ADR-0014 §query_api (delta #10 backfill) |
| Battle HUD (ADR-0015 Accepted 2026-05-03 via /architecture-review delta #10) | `BattleHUD.show_unit_info(unit_id: int) -> void` | RATIFIED parameter-stable per ADR-0015 §4 Public API |
| Battle HUD (ADR-0015 Accepted 2026-05-03 via /architecture-review delta #10) | `BattleHUD.show_tile_info(coord: Vector2i) -> void` | RATIFIED parameter-stable per ADR-0015 §4 Public API |

4-precedent provisional-dependency strategy applies (ADR-0008→0006 / ADR-0012→0009/0010/0011 / ADR-0009→0007 / ADR-0007→Formation Bonus / **this ADR-0005→5 downstream all RATIFIED post-acceptance: ADR-0013 Camera + ADR-0014 Grid Battle + ADR-0015 Battle HUD per delta #10; Settings/Options + Tutorial ADRs remain provisional**). Each downstream ADR can only WIDEN, never NARROW, the locked interface; verified for all 3 ratified downstreams.
...
- **Camera ADR-0013 (Accepted 2026-05-02)**: `Camera.screen_to_grid` + `camera_zoom_min = 0.70` enforcement signatures RATIFIED parameter-stable per ADR-0013 §10 + clamp implementation; no ADR-0005 amendment required (delta #10 backfill).
- **Grid Battle ADR-0014 (Accepted 2026-05-02)**: `is_tile_in_move_range` + `is_tile_in_attack_range` signatures RATIFIED parameter-stable per ADR-0014 §query_api; no ADR-0005 amendment required (delta #10 backfill).
- **Battle HUD ADR-0015 (Accepted 2026-05-03 via /architecture-review delta #10)**: `show_unit_info` + `show_tile_info` signatures RATIFIED parameter-stable per ADR-0015 §4 Public API; no ADR-0005 amendment required.
...
- ADR-0013 BattleCamera (Accepted 2026-05-02) — RATIFIED `screen_to_grid` + `camera_zoom_min = 0.70` enforcement; delta #10 backfill flipped the legacy "Future" marker.
- ADR-0014 GridBattleController (Accepted 2026-05-02) — RATIFIED `is_tile_in_move_range` + `is_tile_in_attack_range`; delta #10 backfill flipped the legacy "Future" marker.
- ADR-0015 BattleHUD (Accepted 2026-05-03 via /architecture-review delta #10) — RATIFIED `show_unit_info` + `show_tile_info` Touch Tap Preview Protocol contract per ADR-0015 §4 Public API.

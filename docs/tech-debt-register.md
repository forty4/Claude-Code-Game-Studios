...
**Update 2026-05-09 (sprint-14 S14-06 codification)**: G-30 codified — headless test pass does not gate windowed-mode lifecycle behavior. Pattern stable at 4 invocations: POLISH-008 (ObjectDB leak) + POLISH-010 (visual rendering) + POLISH-011-as-originally-filed (input non-responsive) + POLISH-011-actual (turn-loop integration gap). Mitigation: windowed-mode smoke harness as separate CI gate (no lint feasible — this is a coverage-design pattern, not a code anti-pattern). The rule file now contains 22 entries.

**Update 2026-05-10 (sprint-15 S15-J + S15-D codification refresh)**: G-30 §Discovered list extended from 4 → 6 invocations within ~12 hours. Invocation #5 = POLISH-012 (CLOSED-LOOP variant: `set_action_controller` DI surface added by S15-A had 0 production callers — production main_scene fell through TEST-SEAM no-op pass for T5; closed at S15-J commit `a4636dc` via mid-sprint amendment + ADR-0014 §Amendment 2026-05-10 #3). Invocation #6 = POLISH-013 META-pattern (the natural-loop integration test designed to CLOSE G-30 verification gap pattern #5 ITSELF surfaces a NEW G-30 instance — 3 fixture iterations all stalled identically; AISystem CONNECT_DEFERRED subscriber never fires in test scope; test-env-vs-prod UNRESOLVED without S15-G windowed re-attestation; S15-D DEFERRED to sprint-16 commit `dca58a4`). Pattern stability advanced 4 → 5 → 6 (sprint-15 alone advanced +2 within <12 hours). Codified with: `production/polish-backlog.md` POLISH-012 + POLISH-013 entries + `production/sprints/sprint-15.md` Mid-Sprint Amendments section + `.claude/rules/godot-4x-gotchas.md` G-30 §Discovered #5 + #6 + sprint-15 retro AI seeded for structural review of G-30 mitigation strategy (test infra alone may not be sufficient; may need input simulation + per-unit T5 timeouts + dedicated G-30 mitigation EPIC). Also same-session: TG-5 codified (`.claude/rules/tooling-gotchas.md`) — SendMessage tool unavailability across 5 agent spawn cycles. The rule file count remains at 22 entries (no NEW gotcha codified — existing G-30 entry refreshed).

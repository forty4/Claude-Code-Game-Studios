...
- **Effective ATK read** (CR-3): `var raw_atk: int = hp_status.get_modified_stat(attacker.unit_id, &"atk")` — provisional stub per ADR-0012 §8 (HP/Status not yet implemented). For this story, use a mock callable injected via test fixture.
- **Effective DEF read** (CR-3): `var stat_name: StringName = &"phys_def" if modifiers.attack_type == PHYSICAL else &"mag_def"` then `var raw_def: int = hp_status.get_modified_stat(defender.unit_id, stat_name)`.

...
| S15-D | **Natural-loop integration test (G-30 mitigation infrastructure)** — author `tests/integration/feature/battle_scene/battle_scene_natural_loop_test.gd` that drives full battle via `initialize_battle` → `_begin_round.call_deferred()` → outcome emit (NOT direct test-seam calls). Closes G-30 verification gap pattern for the turn loop itself. | godot-gdscript-specialist + qa-tester | 0.3d (~2-3h) | S15-A + S15-B + S15-C | (1) Test file exists at specified path; (2) Test drives full battle from `initialize_battle()` to `victory_condition_detected` emit using natural deferred-chain progression (no `_advance_turn` direct calls); (3) Test asserts `round_started(1)` → `unit_turn_started(<first_unit>)` → eventually non-DRAW outcome; (4) Test passes both player + AI turn paths; (5) Test added to CI baseline (e.g., 1288 → 1290+ depending on test function count) |
...
| **R4**: POLISH-011 fix surfaces ADDITIONAL integration gaps beyond the 3 identified (e.g., target_pos vs target_unit_id confusion in AIActionCommand routing; 5-action ActionType mapping ambiguity) | MEDIUM | MEDIUM | S15-D natural-loop integration test catches these by failing on first run; mid-sprint amendment absorbs via Should Have promotion (mirrors sprint-13 S13-11/12 mid-amendment precedent) |
...
- [ ] All Must Have tasks completed (S15-A + S15-B + S15-C + S15-D)

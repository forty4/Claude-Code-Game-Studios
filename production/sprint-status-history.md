...
- 2026-05-09 — **Sprint-12 S12-06 done — §11 USER-OWNED Carryover Threshold Rule added to `docs/process/decisions-convention.md` (HARD GATE at 5th-carry)**. Sprint-11 retro AI #7 + gate-check 2026-05-08 CD recommendation closed via convention extension. **Posture decided** (user concurrence captured at S12-06 plan-time): HARD GATE — at sprint-(N+1) entry, USER-OWNED items at ≥5 carry-counts MUST be resolved via (a) user-attested OR (b) formally cancelled per `production/decisions/{topic-slug}-decision-{date}.md`. **No third path** — "carry one more time" is forbidden once 5th-carry threshold fires. **Location decided**: extend existing `docs/process/decisions-convention.md` with NEW §11 (vs alternative standalone `docs/process/user-owned-carryover-threshold.md` OR `production/decisions/`-style artifact); reuses existing convention infrastructure (templates + triggers + amendment log). **§11 composition** (~80 LoC append between §10 Future Evolution + Amendment log): §11 preamble (USER-OWNED definition + carry-count source `Carryover Backlog` table column "Times Carried" per S11-01 sprint-plan template) + §11.1 Rule (binary path (a) attest / (b) formally cancel; no third path) + §11.2 Why HARD GATE at 5th (3 load-bearing reasons: AI #2 visibility threshold cross at 4 → empirical "stalled" signal at 5; one-project-quarter heuristic at current 3-day-window cadence; HARD GATE preserves refusal-to-fabricate via path (b) explicit cancellation) + §11.2 Why-not soft alternatives (DEFER-TO-PRR rejected for passive-accumulation risk; ASYNC BATCHING rejected for non-binding decision moment; PRODUCER-ESCALATED WORKFLOW REDESIGN deferred to per-item judgement, not universal rule) + §11.3 Live application table (S7-11 5-carry FIRES at sprint-13 entry; S8-15 3-carry below threshold) + §11.4 Reactivation Triggers (T1 RECURRING per-sprint /sprint-plan USER-OWNED carry-count evaluation; T2 /gate-check pre-prod-to-prod tightening — USER-OWNED-≥5 becomes BLOCKING from CONCERNS) + §11.5 Cross-references (10 entries: originating retro/gate-check + sprint-12 plan + sprint-9 retro AI #2 rationale source + sprint-11 retro forecast section + carryover-backlog template + 2 trigger monitoring + 2 live-application target items). **Amendment log entry appended** (3rd amendment to convention; per §6 append-only discipline). **Live application binding implication for sprint-13**: S7-11 USER-OWNED carryover (currently 5th-time at sprint-12 entry) fires HARD GATE at sprint-13 entry — sprint-13 plan MUST list S7-11 with one of (a) user-attested or (b) formal-cancellation-decision-authored as the disposition. S8-15 (3rd-time) continues normal carry; rule does NOT bind yet. **Test baseline preserved**: 1273/1273 PASS / 0 errors / 0 failures / 0 flaky / 0 skipped / 0 orphans / Exit 0; 130/130 test suites; **63rd consecutive failure-free baseline** (was 62nd at S12-08 close). Doc-only addition — no source code change. **Carryover lint sanity check**: `lint_sprint_carryover_count.sh` (S12-09) re-run after edits — Exit 0 (sprint-12 pre-carryover 2 == sprint-11 retro forecast 2; pattern unchanged). **Sprint-12 progression**: Must 2/3 done; **Should 3/3 done ✓** (S12-04 + S12-05 + S12-06 — claude-side Should-Have track COMPLETE); Nice-claude 3/3 done ✓; USER-OWNED 0/2. **Sprint-12 claude-side saturation**: 8-of-8 claude-owned stories closed (Must 2/3 + **Should 3/3** + Nice 3/3 = 8 done; missing Must-Have 1 is S12-03 close-gate rerun which is blocker-bound on USER-OWNED items). Sprint-12 effectively complete on claude-side. **Hygiene streak**: 47 in-patch sprint-status closes (was 46 at S12-08 close; +1 this story). Prior: 2026-05-09 — Sprint-12 S12-08 done — POLISH-006 entry added to polish-backlog.md (lightweight path; Guan Yu + Zhang Fei stubs deferred; 1273/1273 62nd FFB; 46-streak hygiene close).

- 2026-05-09 — **Sprint-12 S12-08 done — POLISH-006 entry added to `production/polish-backlog.md`** (Guan Yu + Zhang Fei character profile stubs DESCOPED carryover from sprint-10 S10-07 → sprint-11 S11-09 Liu Bei first-stub-shipped). **Conditional path resolution**: lightweight (entry-only authoring; no `design/art/characters/guan-yu.md` + `zhang-fei.md` stub authoring this sprint). **Decision rationale**: sprint-12 plan acceptance criterion for S12-08 codified the conditional explicitly — "if no character-art sprint scheduled: EDIT production/polish-backlog.md POLISH-006 entry only (0d outcome); if scheduled: NEW design/art/characters/guan-yu.md + zhang-fei.md (0.2d outcome)". Per gate-check 2026-05-08-rerun §10 line 142 (ADVISORY-1 carried) + sprint-12 plan §Tasks Nice-to-Have S12-08 row, sprint-12 did NOT include character-art commission scope; the lightweight path was the binding choice. Lighter path closes the AD-flagged ADVISORY-CANDIDATE from gate-check 2026-05-08 line 136 ("On the descope-to-1-stub risk") into a tracked Polish-tier entry without committing sprint-12 budget to greenfield character-art authoring. **POLISH-006 entry composition**: 7-field metadata table + Description (3-paragraph context: heroes.json canonical anchors `shu_002_guan_yu` + `shu_003_zhang_fei` + Peach Garden Oath triangle composition cross-reference + sprint-10 retro AI #5 descope-not-cancel pattern) + Action when picked up (specific authoring guidance: Sections 1-3 minimum per Liu Bei stub format + canonical anchors + Pillar 4 minimum recognition triplet + reserved-color discipline + 8 ACs target + 5 OQs deferred + Peach Garden Oath triangle update guidance) + Cross-references (10 entries; gate-check source + carry-forward + originating descope + reference stub + canonical character data + sprint-12 plan row + AD-C5 partial-state closure context + 2 closure-trigger monitoring lines + art-bible cross-binding). **Tier**: ADVISORY (graduates to BLOCKING-at-character-art-sprint-entry: stubs MUST land before any character-art commission sprint closes story-readiness; ALSO graduates to BLOCKING-at-Polish-gate per gate-check rerun line 142 "must resolve before Polish gate"). **Closure trigger**: (a) FIRST: any sprint plan adds a character-art commission story (forcing function — stubs must precede commission); OR (b) `production/stage.txt` content equals `Polish` (Polish gate hard-blocker per gate-check rerun); whichever fires first. **3 indexes regenerated**: by-Status (Open count 5 → 6), by-Source (NEW row "Gate-check 2026-05-08 ADVISORY-CANDIDATE (carried into 2026-05-08-rerun ADVISORY-1)"), by-Closure-Trigger (NEW row "Character-art commission sprint enters planning (forcing function) OR Polish gate"). **Amendment log entry appended** to polish-backlog.md per S11-06 amendment-log discipline (append-only). **Test baseline preserved**: 1273/1273 PASS / 0 errors / 0 failures / 0 flaky / 0 skipped / 0 orphans / Exit 0; 130/130 test suites; **62nd consecutive failure-free baseline** (was 61st at S12-09 close). Doc-only addition — no source code change, no test count change. **Hygiene streak**: 46 in-patch sprint-status closes (was 45 at S12-09 close; +1 this story). **Sprint-12 progression**: Must 2/3 done (S12-01 + S12-02; S12-03 close-gate rerun blocked on USER-OWNED S12-10/11); Should 2/3 done (S12-04 + S12-05); **Nice-claude 3/3 done** ✓ (S12-07 + S12-08 + S12-09 — claude-side Nice-to-Have track COMPLETE); USER-OWNED 0/2. **Sprint-12 claude-side saturation**: with S12-08 close-out, sprint-12's claude-owned story track is fully closed at 7/7 (Must 2/3 + Should 2/3 + Nice 3/3). Remaining open items are (a) S12-03 close-gate rerun — depends on USER-OWNED attestations to flip CONCERNS → PASS; (b) S12-06 USER-OWNED 5th-time threshold codification — needs user concurrence; (c) S12-10 + S12-11 — USER-OWNED attestations. **In-patch byte-cap fix** (sprint-11 retro "What Went Poorly" item 1 recurrence): first-draft per-story changelog line measured 205 bytes (over 200 cap); trimmed via "POLISH-006 added" → "POLISH-006 added" pattern preserve + "no character-art sprint scheduled" → "(lightweight path; no character-art sprint scheduled)" + drop "to commission-sprint forcing function" suffix. Final 196 bytes ✓. Pattern stable at 4 invocations (sprint-11 S11-05/07/09+10+11 + sprint-12 S12-08); codification candidate at sprint-12 retro IF the recurrence frequency rises further. Prior: 2026-05-09 — Sprint-12 S12-09 done — lint_sprint_carryover_count.sh authored + CI wired (PASS sprint-12 pre-carryover=2 vs sprint-11 retro forecast=2; <4 threshold; 1273/1273 61st FFB).

- 2026-05-09 — **Sprint-12 S12-09 done — `tools/ci/lint_sprint_carryover_count.sh` authored + CI wired**. Sprint-11 retro AI #5 optional automation candidate shipped (was Low priority "optional; sprint-13+ if AI #2 still binding" — opportunistically shipped same-sprint as S12-07 follow-on closure-mode discipline). **What**: ~165-line bash lint script that finds the latest `production/sprints/sprint-N.md` plan, counts carryover rows in the `## Carryover Backlog` (or pre-codification `## Carryover from Previous Sprint`) section table, cross-verifies against the prior sprint's retro forecast count via the canonical "Carryover concentration into sprint-{N}: **X**" pattern (sprint-11+ form) with fallback to "X USER-OWNED" + "Y claude-owned" sum (pre-canonical retros), and flags ≥4 visibility threshold breach per sprint-9 retro AI #2 (sustained sprint-10/11/12). **Section header compatibility**: handles both `## Carryover Backlog` (sprint-12+; codified at S11-01) and `## Carryover from Previous Sprint` (sprint-11 and earlier). **Row pattern**: counts table rows matching `^| **S<N>-<NN>**` (bold-anchored story IDs) with fallback to unbolded `^| ... S<N>-<NN>` rows (sprint-11 form). **Retro forecast extraction**: primary regex anchored on `Carryover concentration into sprint-{LATEST_SPRINT}` literal phrase + sed-strip the prefix + grep first leading `**N` or `N` digit-run (avoids the bug where unanchored `\*\*[0-9]+` matched bold numbers earlier in the same line — e.g., sprint-11 retro line `**9 of 9 absorbed.** Carryover concentration into sprint-12: **2 USER-OWNED only**.` would match `**9` instead of `**2` without anchoring). Fallback: awk-scan within a 8-line window after `forecast carryover` or `Carryover from prior sprint` header for `([0-9]+) USER-OWNED` + `([0-9]+) claude` substrings + sum. ADVISORY-only when no pattern matches (forward-compat for pre-canonical retros). **Verification on current state**: `bash tools/ci/lint_sprint_carryover_count.sh` returns Exit 0 with output `pre-carryover count: 2 / retro forecast count: 2 / PASS: sprint-12 pre-carryover (2) == sprint-11 retro forecast (2)` — matches sprint-12 plan acceptance criterion ("Exit 0 on current sprint-11 → sprint-12 carryover (2 USER-OWNED, well below threshold)"). **CI wiring**: appended `Lint sprint carryover count vs prior retro forecast (sprint-11 retro AI #5 / S12-09)` step to `.github/workflows/tests.yml` after the S12-04 story-status-consistency lint step + before the GdUnit4 tests step (per S12-04 lint-cluster precedent). **Exit code semantics**: 0 = PASS (count consistent + below threshold OR retro unextractable + count below threshold) / 1 = FAIL (count mismatch) / 2 = WARN reserved for future hardening (currently non-blocking; threshold-only-breach exits 0 with PASS-WITH-WARN message). **In-patch fix during authoring**: first run reported FAIL with retro forecast=9 (mis-extracted from the bolded `**9 of 9 absorbed.**` prefix on the same line); fix applied via sed-strip prefix anchored on canonical phrase before number extraction. Pattern stable at 1 invocation; codification candidate at sprint-12 retro: "regex extraction patterns must be ANCHORED on the literal phrase that precedes the target token, not relying on positional `head -1` after `grep -oE` of a generic pattern." **Test baseline preserved**: 1273/1273 PASS / 0 errors / 0 failures / 0 flaky / 0 skipped / 0 orphans / Exit 0; 130/130 test suites; **61st consecutive failure-free baseline** (was 60th at S12-07 close). Doc/CI/lint-only addition — no source code change. **Hygiene streak**: 45 in-patch sprint-status closes (was 44 at S12-07 close; +1 this story). **Sprint-12 progression**: Must 2/3 done (S12-01 + S12-02; S12-03 close-gate rerun blocked on USER-OWNED S12-10/11); Should 2/3 done (S12-04 + S12-05); Nice-claude 2/3 done (S12-07 + S12-09); USER-OWNED 0/2. **Lint inventory grows**: project now has 73 CI lint scripts in `tools/ci/` (was 72 at S12-04 close; +1 this story). Prior: 2026-05-09 — Sprint-12 S12-07 done — HYBRID closure-mode sprint pattern adopted (≥3-of-5 signal trigger; sprint-11 retro AI #6 closed; 1273/1273 60th FFB; 44-streak hygiene close).

- 2026-05-09 — **Sprint-12 S12-07 done — HYBRID closure-mode sprint pattern adopted (≥3-of-5 signal trigger)**. Sprint-11 retro AI #6 closed via binding process-decision artifact `production/decisions/closure-mode-sprint-pattern-2026-05-09.md` per `docs/process/decisions-convention.md` Route c. **Decision**: closure-mode sprints (≥80% closure / admin / codification work) trigger when ≥3 of 5 signals fire at `/sprint-plan` time: (A) carryover concentration ≥4 OR ≥4-time-carryover item / (B) unpaid codification AIs from prior retro / (C) ≥3 epics flipped Complete in last 2 sprints / (D) hygiene drift accumulation (≥5 TODOs OR ≥10 lint drifts OR ≥1 vacuous-pass lint) / (E) no fresh must-have feature ready (greenfield needs design-spec authoring first). Otherwise mixed-mode is the default. Producer evaluates signals at every `/sprint-plan` invocation; designation outcome recorded in NEW `## Sprint Mode` section to be added to `.claude/skills/sprint-plan/SKILL.md` Phase 2 template at first sprint-13+ plan invocation (Trigger 4 deferred companion edit). **Why HYBRID over EMBRACE-as-cadence or RESERVE-as-stopgap**: 4 load-bearing reasons — (1) sprint-11 ÷4.4 multiplier vs ÷3 mixed-mode baseline shows planned closure-mode is distinctively faster, but ONLY when signals justify it (cadence would force closure-mode into low-signal sprints, leaving the multiplier theoretical); (2) all 5 signals are mechanically greppable per `docs/process/decisions-convention.md` §4 trigger discipline; (3) ≥3-of-5 threshold calibrated to sprint-11 empirical floor (sprint-11 fired exactly 4 of 5 signals; sprints 7-10 fired ≤2 each); (4) producer override authority preserved via Trigger 2 (rolling 3-sprint override-divergence window). **What is NOT decided**: no specific cadence; no closure-mode allocation cap/floor; no change to mixed-mode default workflow; no velocity multiplier model amendment; no skill creation; no retroactive application to sprint-12. **4 reactivation triggers**: (T1) RECURRING sprint-(N+1) `/sprint-plan` signal evaluation — the standing per-sprint trigger that operationalizes the rule; (T2) override-divergence ≥2 of 3 consecutive sprints triggers signal re-calibration; (T3) velocity-multiplier amendment in any retro re-validates §3.6 cost-benefit; (T4) `## Sprint Mode` template addition at first post-decision `/sprint-plan` invocation. **Cost-benefit**: HYBRID estimated ~20-25% closure-mode allocation (vs EMBRACE ~33% over-allocation OR RESERVE ~10-15% under-allocation); aligned with project's broader data-driven discipline. **Test baseline preserved**: 1273/1273 PASS / 0 errors / 0 failures / 0 flaky / 0 skipped / 0 orphans / Exit 0; **60th consecutive failure-free baseline** (was 59th at S12-05 close); 130/130 test suites. Doc-only addition — no source code change. **Hygiene streak**: 44 in-patch sprint-status closes (was 43 at S12-05 close; +1 this story). **Sprint-12 progression**: Must 2/3 done (S12-01 + S12-02; S12-03 close-gate rerun blocked on USER-OWNED S12-10/11); Should 2/3 done (S12-04 + S12-05); Nice-claude 1/3 done (S12-07 this); USER-OWNED 0/2. **Meta-observation**: `production/decisions/` now contains 4 artifacts (ci-lane-gap from sprint-10 + pillar-3-deferral + prototype-as-gate-sufficient + closure-mode-sprint-pattern from sprint-12) across 2 distinct sprints. **`docs/process/decisions-convention.md` §7 skill-promotion trigger has FIRED** (≥3 artifacts AND ≥2 distinct sprints) — separate sprint-12 retro AI candidate (NEW #12) for skill-promotion evaluation; NOT closed by this S12-07 decision. Prior: 2026-05-09 — Sprint-12 S12-05 done — TODO triage 5→2 (4 surgical edits across 3 files; 1273/1273 59th FFB; 43-streak hygiene close).

- 2026-05-09 — **Sprint-12 S12-05 done — TODO triage Address actions bundle (5 → 2)**. Sprint-11 retro AI #4 + S11-11 process-audit follow-through. 5 src/ TODOs categorised + 4 surgical edits applied across 3 files: (1) `save_manager.gd:200` doc-comment "(migration shim — see TODO below)" REMOVED — stale phrase from pre-save-load-epic-implementation era; replaced with accurate "SaveMigrationRegistry.migrate_to_current(ctx) — apply schema migrations / GameBus.save_loaded.emit(migrated); return migrated"; (2) `payloads/beat_cue.gd:6` `## TODO:` → `## NOTE:` REFORMAT — stub-class file with no logic; future-Story-Event-GDD-#10 marker is informational not actionable; reformat drops it from `grep TODO` count; (3) `grid_battle_controller.gd:342` `# TODO(story-003+): populate FSM state, unit positions, acted flags` REWORD — story-003+ ref stale (story shipped); accurate ref is Battle AI ADR (sprint-7+; NOT YET WRITTEN) per ADR-0014 line 598 + GDD; new wording: `# TODO (Battle AI ADR — not yet written; sprint-7+): populate FSM state, unit positions, acted flags. Shape locked at ADR authoring time per ADR-0014 §10 Key Interfaces (opaque shape) + ADR-0014 line 598.`; (4) `grid_battle_controller.gd:424` `# TODO(story-006): reset per-turn acted flag for this unit` REMOVED — fully stale; the function below it does AI-action emission (story-006 actually shipped that), NOT acted-flag reset which happens in `end_player_turn()` line 357 (`_acted_this_turn.clear()`) and round_started handler. Comment didn't describe what the function does — leftover from a discarded design. The next-line comment "Per ADR-0019 + grid-battle.md CR-3: AI-turn detection + ai_action_requested emission." is the correct doc-comment. **Disposition summary**: 2 KEEP (TODO-01 + TODO-04 — both genuine future-work deferrals with ADR + TR refs); 2 REMOVE (TODO-02 stale phrase + TODO-05 stale comment); 1 REFORMAT (TODO-03 TODO→NOTE for future-epic stub). **Result**: TODO count 5 → 2 ✓ matches sprint-status.yaml S12-05 spec target. **Test baseline preserved**: 1273/1273 PASS / 0 errors / 0 failures / 0 flaky / 0 skipped / 0 orphans / Exit 0; **59th consecutive failure-free baseline** (was 58th at S12-04 close); 130/130 test suites executed. Comment-only changes — no logic affected, no test count change. **Hygiene streak**: 43 in-patch sprint-status closes (was 42 at S12-04 close; +1 this story). **Sprint-12 progression**: Must 2/3 done (S12-01 + S12-02; S12-03 close-gate rerun blocked on S12-10/11 USER attestations); Should 2/3 done (S12-04 + S12-05); Nice-claude 0/3; USER-OWNED 0/2. Prior: 2026-05-08 — Sprint-12 S12-04 done — `lint_story_status_consistency` 33-drift bulk cleanup + CI wire (1273/1273 58th FFB; 42-streak hygiene close).

- 2026-05-08 — **Sprint-12 S12-04 done — `lint_story_status_consistency` 33-drift bulk cleanup + CI wire**. Sprint-11 retro AI #3 follow-through. 33 lint failures collapsed to **4 surgical edits** after categorisation: (1) `production/epics/balance-data/EPIC.md` Status header `Ready` → `Complete (2026-05-01)` propagated to 5 stories; (2) `production/epics/turn-order/EPIC.md` Status header `Ready` → `Complete (2026-05-02)` propagated to 7 stories; (3) `production/epics/index.md` damage-calc row Stories cell + Status cell updated to `**11/11 Complete** 🎉` / `**Complete** (2026-04-27)` propagated to 11 stories; (4) `production/epics/index.md` input-handling row Stories cell + Status cell updated to `**10/10 Complete** 🎉` / `**Complete** (2026-05-07)` propagated to 10 stories. CI wiring same-patch: `.github/workflows/tests.yml` step `Lint story Status consistency across 4 canonical sources` appended after the 3 save-load story-003 lint steps. Verification: `tools/ci/lint_story_status_consistency.sh` `FAIL — 33 stories` → `PASS — all Complete stories have consistent downstream Status`. **Test baseline preserved**: 1273/1273 PASS / 0 errors / 0 failures / 0 flaky / 0 skipped / 0 orphans / Exit 0; **58th consecutive failure-free baseline** (was 57th at S12-02 epic-terminal close). 130/130 test suites executed. **Hygiene streak**: 42 in-patch sprint-status closes (was 41 at S12-02 close; +1 this story). **Sprint-12 progression**: Must 2/3 done (S12-01 + S12-02); Should 1/3 done (S12-04); Nice-claude 0/3; USER-OWNED 0/2. Prior: 2026-05-08 — Sprint-12 S12-02 SHIPPED — chapter-prototype-demo Demo-epic 1/1 epic-terminal close (gate-check item 3 closed; 1273/1273 57th FFB).

- 2026-05-08 — **Sprint-12 S12-02 SHIPPED — chapter-prototype-demo Demo-epic 1/1 epic-terminal close**. First Demo-layer epic in the project; first prototype-side `/quick-design` → `/create-stories` → `/dev-story` chain shipped end-to-end. Pillar 4 (삼국지의 숨결) atmospheric moment (운명 분기 reserved-color treatment 주홍 `#C0392B` panel tint at 0.35α + 금색 `#D4A017` title color + synthesized 묵 hum audio cue + 1.5s ceremonial dwell lockout per AC-SP-9) now mechanically observable on REWRITTEN branch resolution in `prototypes/chapter-prototype/`. **Gate-check 2026-05-08 path-to-PASS item 3 CLOSED** (≥1 player-facing Pillar 3/4 demonstration). 7 ACs PASS / 1273/1273 tests / **57th consecutive failure-free baseline** (was 56th at save-load story-003 close). Sprint-12 Must 2/3 done (S12-01 save-load + S12-02 demo); S12-03 close-gate rerun candidate now eligible (depends on S12-10 + S12-11 user attestations to flip CONCERNS → PASS). Prior: 2026-05-08 — save-load story-003 epic-terminal Complete (13 tests; 1266/1266 PASS 56th FFB; 3 lints + verif summary); SAVE-LOAD 3/3 🎉; #17 Implemented.

### S12-06 — §11 USER-OWNED Carryover Threshold Rule added to `docs/process/decisions-convention.md` (HARD GATE at 5th-carry)

- 2026-05-09 — **done**: sprint-11 retro AI #7 + gate-check 2026-05-08 CD recommendation closed via decisions-convention.md §11 extension. **Pre-authoring user concurrence captured** (S12-06 plan-time question): user chose **HARD GATE** posture (vs DEFER-TO-PRR / ASYNC BATCHING / PRODUCER-ESCALATED WORKFLOW REDESIGN alternatives) AND **extend decisions-convention.md** location (vs standalone process doc / production/decisions/ artifact). **§11 location**: between §10 Future Evolution and the Amendment log; numbered §11 + §11.1-§11.5 sub-sections. **Section composition**: §11 preamble defines USER-OWNED carryover (item cannot proceed without user attestation, paid prereqs, or credentials) + carry-count source (Carryover Backlog "Times Carried" column per S11-01 sprint-plan template) + the rule's binding moment (sprint-(N+1) entry). §11.1 Rule states the binary path: (a) user-attested via standard /story-done flow; OR (b) formally cancelled via NEW process-decision artifact with binding outcome `CANCEL — not pursued`. **No third path** is permitted once 5th-carry threshold fires — "carry one more time" is forbidden. §11.2 enumerates 3 load-bearing reasons for picking 5 (vs 3 / 7 / soft-deferred): (1) sprint-9 retro AI #2 ≥4 visibility threshold + 5th carry as empirical stalled-signal; (2) ~one-project-quarter heuristic at 3-day-window cadence; (3) HARD GATE preserves refusal-to-fabricate via path-(b) cancellation while preventing soft-stall. Soft alternatives (DEFER-TO-PRR / ASYNC BATCHING / PRODUCER-ESCALATED WORKFLOW REDESIGN) explicitly rejected with rationale: passive-accumulation risk, non-binding decision moment, too-heavy-for-universal-rule respectively. §11.3 Live application table maps current state to rule outcome — S7-11 (5-carry; project record per sprint-12 plan Carryover Backlog) FIRES at sprint-13 entry; S8-15 (3-carry; below threshold) continues normal carry. §11.4 Reactivation Triggers (2): (T1) RECURRING — every /sprint-plan invocation runs USER-OWNED carry-count evaluation; (T2) /gate-check pre-prod-to-prod TIGHTENING — USER-OWNED-≥5 items become BLOCKING (was CONCERNS) at any subsequent gate-check evaluation; this is a strictness step-up from prior gate-check semantics. §11.5 Cross-references (10 entries): originating retro AI #7 line 160; originating gate-check 2026-05-08 CD recommendation; sprint-12 S12-06 plan task row; sprint-9 retro AI #2 rationale source (load-bearing for 5-not-3-not-7 calibration); sprint-11 retro forecast section (S11-12 → S12-10 5th-time pre-attestation; S11-13 → S12-11 3rd-time); carryover-backlog template (sprint-plan SKILL.md Phase 2 codified at S11-01); 2 trigger-monitoring lines; 2 live-application target items (S7-11 prototypes/chapter-prototype/REPORT.md + S8-15 production/qa/qa-signoff-sprint-8-2026-05-06.md). **Amendment log**: 3rd entry appended to decisions-convention.md per §6 append-only discipline (1st = 2026-05-08 initial codification; 2nd = NONE since 2026-05-08 — wait, this is actually the 2nd amendment after initial since no other amendments existed; correcting: 2nd entry total). **Live application binding implication for sprint-13 plan authoring**: S7-11 USER-OWNED carryover at 5th carry-count fires HARD GATE at sprint-13 entry. Sprint-13 plan MUST list S7-11 with one of these dispositions: (a) "user-attested" (claude-side: standard /story-done close-out flow with user attestation evidence captured) OR (b) "formally cancelled per `production/decisions/s7-11-vs-validation-cancel-decision-{date}.md`" (claude-side: producer + user co-author cancellation rationale per convention §3 template; binding outcome CANCEL — not pursued; rationale must address why no graduation to BLOCKING + what UX implication is + which downstream tests/ACs need re-routing). NOT permitted: (c) "carry to sprint-14" — rule binds. S8-15 (3rd-time) is below threshold; continues normal carry; rule does NOT bind for S8-15 yet. **Test baseline preserved**: 1273/1273 PASS / 0 errors / 0 failures / 0 flaky / 0 skipped / 0 orphans / Exit 0; 130/130 test suites; **63rd consecutive failure-free baseline** (was 62nd at S12-08 close). Doc-only addition — no source code change. **Carryover lint sanity check**: `bash tools/ci/lint_sprint_carryover_count.sh` (S12-09 lint) re-run after S12-06 edits — Exit 0 (sprint-12 pre-carryover 2 == sprint-11 retro forecast 2; carryover-backlog table content unchanged; rule binding only takes effect at sprint-13 entry). **Sprint-12 progression delta**: Must 2/3 done (S12-01 + S12-02; S12-03 close-gate rerun blocker-bound on USER-OWNED); **Should 3/3 done ✓** (S12-04 + S12-05 + S12-06 — claude-side Should-Have track COMPLETE); Nice-claude 3/3 done ✓ (S12-07 + S12-08 + S12-09); USER-OWNED 0/2. **Sprint-12 claude-side saturation milestone (final)**: 8-of-8 claude-owned stories closed (Must 2/3 + Should 3/3 + Nice 3/3 = 8 done). Sprint-12 effectively COMPLETE on claude-side; only S12-03 (Must, blocker-bound) + S12-10/11 (USER-OWNED) remain. **Hygiene streak**: 47 in-patch sprint-status closes (was 46 at S12-08 close; +1 this story). **Sprint-13 plan implication codified**: AI #7 closure at S12-06 means sprint-13 plan inherits a HARD-GATE-bound disposition obligation for S7-11. The producer authoring sprint-13 plan must run carryover-backlog evaluation FIRST (per §11.4 Trigger 1) + flag S7-11 as 5th-carry + propose path (a) or path (b) as binding scope. **Decisions-convention growth**: convention is now ~315 LoC (was ~238 at sprint-11 S11-05 codification; +77 LoC for §11). Future evolution per §10 still applies — convention may further grow as more rules accumulate. **Cross-system effect**: this is the first universal-rule-extension to the convention since its codification at sprint-11 S11-05. Establishes precedent that the convention extends via numbered top-level sections (§11, §12, ...) rather than embedding rules in §3 template — keeps the template stable for new artifact authoring while allowing rule layer to grow independently. **AI seed for sprint-12 retro**: NEW #15 candidate — convention-extension-via-numbered-section pattern (vs template-modification pattern); validate at next convention extension whether this is the right shape. **Sprint-13 entry pre-flight**: producer should evaluate S7-11 disposition BEFORE sprint-13 /sprint-plan invocation; if user opts for path (b) cancellation, the cancellation decision artifact authoring is itself a sprint-13 Must-Have task. **Original byte count**: ~470 bytes per-story changelog target (truncated to ~190-byte line in YAML).

### S12-08 — POLISH-006 entry added to `production/polish-backlog.md` (Guan Yu + Zhang Fei character profile stubs DESCOPED carryover)

- 2026-05-09 — **done**: AD-flagged ADVISORY-CANDIDATE from gate-check 2026-05-08 line 136 ("On the descope-to-1-stub risk") + ADVISORY-1 carried into gate-check 2026-05-08-rerun line 142 closed via lightweight conditional path (entry-only authoring at `production/polish-backlog.md`; no `design/art/characters/guan-yu.md` + `zhang-fei.md` stub authoring this sprint). **Conditional resolution**: sprint-12 plan acceptance criterion explicitly codified the binary — "if no character-art sprint scheduled: EDIT polish-backlog.md POLISH-006 entry only (0d outcome); if scheduled: NEW guan-yu.md + zhang-fei.md (0.2d outcome)". Sprint-12 did NOT include character-art commission scope (verified by grep across `production/sprints/sprint-12.md` Tasks tables — no character-art commission story; the only character-related work was the Liu Bei stub already shipped at S11-09). Lightweight path = binding choice. **POLISH-006 entry composition** (~80 LoC append to polish-backlog.md §Live Entries before the §Index sections): 7-field metadata table + Description (3-paragraph context) + Action when picked up + 10-entry Cross-references list. **Description anchors**: Guan Yu = `shu_002_guan_yu` (관우 / 关羽 / 云长 per `assets/data/heroes/heroes.json`) + Zhang Fei = `shu_003_zhang_fei` (장비 / 张飞 / 翼德); both COMMANDER class; both `bond_oath_peach_garden` SWORN_BROTHER symmetric with Liu Bei + each other (the triangle is structurally incomplete until both stubs ship); reference stub is `design/art/characters/liu-bei.md` (silhouette-distinguishability benchmark from sprint-11 S11-09). **Action guidance**: Sections 1-3 minimum (silhouette + costume + role-anchor) + canonical heroes.json anchors + Pillar 4 minimum recognition triplet (32px silhouette distinguishability) + reserved-color discipline (주홍 `#C0392B` + 금색 `#D4A017` reserved for destiny-branch reveals only) + Peach Garden Oath triangle update guidance with traditional iconography hooks (Guan Yu = 偃月刀 halberd + 美髯公 full beard; Zhang Fei = 蛇矛 spear + bristled-hog beard + dark complexion) + 8 ACs target + 5 OQs deferred (mirroring Liu Bei stub format). **Tier**: ADVISORY (graduates to BLOCKING-at-character-art-sprint-entry: stubs MUST land before any character-art commission sprint closes story-readiness; ALSO graduates to BLOCKING-at-Polish-gate per gate-check rerun line 142 "must resolve before Polish gate"). **Closure trigger**: (a) FIRST: any sprint plan adds a character-art commission story (forcing function — stubs must precede commission); OR (b) `production/stage.txt` content equals `Polish` (Polish gate hard-blocker per gate-check rerun); whichever fires first. **3 indexes regenerated**: by-Status (Open count 5 → 6; POLISH-006 appended to ID list), by-Source (NEW row "Gate-check 2026-05-08 ADVISORY-CANDIDATE (carried into 2026-05-08-rerun ADVISORY-1)" added — distinguishes from Battle HUD epic verification source for POLISH-001..005), by-Closure-Trigger (NEW row "Character-art commission sprint enters planning (forcing function) OR Polish gate (`production/stage.txt` = `Polish`)"). **Amendment log entry appended** to polish-backlog.md per S11-06 amendment-log discipline (append-only — never modify body sections after initial date; this is the 2nd amendment after the 2026-05-08 initial codification). **Cross-system effect**: Polish-tier ledger now has **6 Open entries** (was 5 at S11-06 close-out); first non-Battle-HUD-sourced entry; first entry with character-art-sprint-trigger condition; first entry with dual closure trigger (forcing function OR stage flip whichever first). Establishes precedent for future gate-check-flagged ADVISORY-CANDIDATEs to land as polish-backlog entries when conditional path resolves to lightweight. **AD-C5 partial-state closure remains stable**: gate-check 2026-05-08 closed AD-C5 to "first-stub-shipped" via Liu Bei (sprint-11 S11-09); POLISH-006 entry preserves this state — the AD-C5 ADVISORY does not re-open at this entry-only authoring (which is intentionally lightweight, not a substitute for the actual stubs). AD-C5 will fully close when both Guan Yu + Zhang Fei stubs ship; until then, gate-check ADVISORY-1 carry-forward continues. **Test baseline preserved**: 1273/1273 PASS / 0 errors / 0 failures / 0 flaky / 0 skipped / 0 orphans / Exit 0; 130/130 test suites; **62nd consecutive failure-free baseline** (was 61st at S12-09 close). Doc-only addition — no source code change. **Carryover lint sanity check**: `bash tools/ci/lint_sprint_carryover_count.sh` (S12-09 lint) re-run after S12-08 edits — Exit 0 (sprint-12 pre-carryover 2 == sprint-11 retro forecast 2; pattern unchanged). **Sprint-12 progression delta**: Must 2/3 done (S12-01 + S12-02); Should 2/3 done (S12-04 + S12-05); **Nice-claude 3/3 done ✓** (S12-07 closure-mode + S12-08 POLISH-006 + S12-09 carryover-count lint — claude-side Nice-to-Have track COMPLETE); USER-OWNED 0/2. **Sprint-12 claude-side saturation milestone**: 7-of-7 claude-owned stories closed (Must 2/3 + Should 2/3 + Nice 3/3 = 7 done; the missing Must-Have 1 is S12-03 close-gate rerun which is blocker-bound on USER-OWNED items). **Hygiene streak**: 46 in-patch sprint-status closes (was 45 at S12-09 close; +1 this story). **In-patch byte-cap fix during close-out**: first-draft per-story changelog line measured 205 bytes (over 200-byte cap); trimmed via inline edit ("POLISH-006 added to polish-backlog.md (no character-art sprint scheduled); Guan Yu + Zhang Fei stubs deferred to commission-sprint forcing function" → "POLISH-006 added to polish-backlog.md (lightweight path; no character-art sprint scheduled); Guan Yu + Zhang Fei stubs deferred"). Final 196 bytes ✓. Pattern is the **4th invocation** of the byte-cap-recurrence trap (sprint-11 retro "What Went Poorly" item 1 listed 3 invocations at sprint-11 close: S11-05 + S11-07 + S11-09+10+11). Pattern stable at 4 invocations; codification candidate at sprint-12 retro IF the recurrence frequency continues to rise (currently within sprint-11 retro's "not material but recurring micro-friction" framing). **AI seed for sprint-12 retro**: NEW #14 candidate — byte-cap-recurrence-prevention codification: draft per-story changelog phrasing in scratch space + byte-check before commit-into-story-row (mitigation already proposed in sprint-11 retro; codification gap is making the mitigation a `/story-done` Phase 7 step rather than an oral guideline). **Open claude-only sprint-12 work after S12-08**: ZERO. Remaining items are S12-03 (blocker-bound) + S12-06 (user-concurrence-bound) + S12-10 + S12-11 (USER-OWNED). **Cross-references**: gate-check 2026-05-08 line 136 NEW ADVISORY-CANDIDATE (originating); gate-check 2026-05-08-rerun line 142 ADVISORY-1 carried; sprint-10 retro AI #5 (DESCOPE 3-stub → 1-stub decision); sprint-11 S11-09 close-out (Liu Bei first-stub-shipped); reference stub `design/art/characters/liu-bei.md`; canonical character data `assets/data/heroes/heroes.json`; sprint-12 plan S12-08 row; polish-backlog.md POLISH-006 entry (this commit); polish-backlog.md amendment log 2nd entry. **Original byte count**: ~440 bytes per-story changelog target (truncated to ~196-byte line in YAML).

### S12-09 — `tools/ci/lint_sprint_carryover_count.sh` authored + CI wired

- 2026-05-09 — **done**: sprint-11 retro AI #5 optional automation candidate (was Low priority; "optional; sprint-13+ if AI #2 still binding") shipped opportunistically same-sprint as S12-07 closure-mode follow-on. **Lint script** (~165 LoC at `tools/ci/lint_sprint_carryover_count.sh`): finds latest `production/sprints/sprint-N.md`, counts carryover rows, cross-verifies against prior sprint retro's forecast, flags ≥4 visibility threshold breach (sprint-9 retro AI #2; sustained sprint-10/11/12). **Section header compatibility** (TG-3-aware flag/next pattern per `.claude/rules/tooling-gotchas.md` cross-reference): handles `## Carryover Backlog` (sprint-12+ codified form per S11-01) AND `## Carryover from Previous Sprint` (sprint-11 and earlier pre-codification form). **Row pattern**: primary `^| **S<N>-<NN>**` bolded-ID match; fallback `^| .* S<N>-<NN>` unbolded match (sprint-11 form). **Retro forecast extraction strategy**: (1) primary regex anchored on the literal phrase `Carryover concentration into sprint-{LATEST_SPRINT}` — sed-strips that prefix + grep first leading `**N` or `N` digit-run; (2) fallback awk-scan within 8-line window after `forecast carryover` OR `Carryover from prior sprint` header for `([0-9]+) USER-OWNED` + `([0-9]+) claude` substrings + sum; (3) ADVISORY-only when no pattern matches (forward-compat for pre-canonical retros where forecast metric was inline rather than headed). **Exit codes**: 0 = PASS (count consistent + below threshold OR retro unextractable but count below threshold); 1 = FAIL (count mismatch between plan + retro forecast); 2 = WARN reserved for future hardening (threshold-only-breach currently exits 0 with PASS-WITH-WARN message — non-blocking). **First-run failure + in-patch fix**: initial regex `\*\*[0-9]+` (no anchor) misread sprint-11 retro line `**9 of 9 absorbed.** Carryover concentration into sprint-12: **2 USER-OWNED only**.` — matched `**9` from "**9 of 9 absorbed.**" prefix instead of the target `**2`. Fix: anchored extraction on the literal `Carryover concentration into sprint-{N}` phrase + sed-stripped prefix before number-run match. **Codification candidate at sprint-12 retro** (NEW #13 candidate): "regex-based-extraction patterns from prose-rich retro/changelog text must be ANCHORED on the literal phrase preceding the target token, not relying on positional `head -1` after `grep -oE` of a generic format pattern." Pattern stable at 1 invocation but echoes G-9 (`%` operator binds to immediate left operand) + G-24 (`as` operator low-precedence trap) — generic operator-precedence-and-anchoring family of gotchas. **Verification on current sprint-12 state**: lint output: `lint_sprint_carryover_count: latest sprint = sprint-12 / source plan: production/sprints/sprint-12.md / pre-carryover count: 2 / prior retro: production/retrospectives/retro-sprint-11-2026-05-08.md / retro forecast count: 2 / retro pattern matched: canonical (Carryover concentration into sprint-N) / PASS: sprint-12 pre-carryover (2) == sprint-11 retro forecast (2) / lint_sprint_carryover_count: PASS / exit=0`. Matches sprint-12 plan acceptance criterion verbatim ("Lint script at `tools/ci/lint_sprint_carryover_count.sh` Exit 0 on current sprint-11 → sprint-12 carryover (2 USER-OWNED, well below threshold)"). **CI wiring**: new `Lint sprint carryover count vs prior retro forecast (sprint-11 retro AI #5 / S12-09)` step appended to `.github/workflows/tests.yml` after the S12-04 `Lint story Status consistency` step + before the GdUnit4 tests step (mirrors S12-04 lint-cluster placement). The CI sequence in the lint cluster is now: `lint_save_resource_loader_cache_mode_ignore.sh` → `lint_save_migration_callable_purity.sh` → `lint_save_context_export_discipline.sh` → `lint_story_status_consistency.sh` → **`lint_sprint_carryover_count.sh`** (NEW) → GdUnit4 tests. **Forward-looking nature**: this lint joins `lint_save_migration_callable_purity.sh` as the project's 2nd "vacuous-pass-on-current-state" lint (per sprint-12 retro AI #10 from active.md handoff: "Vacuous-pass lint precedent — heuristic lints with vacuous-pass conditions"). For sprint-12, the lint is non-vacuous (current state PASSes meaningfully against retro forecast); the vacuous-pass risk arises if sprint-(N+1) retros stop using the canonical phrase + the lint downgrades to ADVISORY-only mode. Explicit retro-pattern stability monitoring is producer responsibility per sprint-12 retro AI #5 (closure gate-check evaluation companion). **Test baseline preserved**: 1273/1273 PASS / 0 errors / 0 failures / 0 flaky / 0 skipped / 0 orphans / Exit 0; 130/130 test suites; **61st consecutive failure-free baseline** (was 60th at S12-07 close). **Sprint-12 progression delta**: Must 2/3 done (S12-01 + S12-02); Should 2/3 done (S12-04 + S12-05); Nice-claude 2/3 done (S12-07 + S12-09 this); USER-OWNED 0/2. **Hygiene streak**: 45 in-patch sprint-status closes (was 44 at S12-07 close; +1 this story). **Lint inventory growth**: project now has 73 CI lint scripts in `tools/ci/` (was 72 at S12-04 close; +1 this story). **Cross-references**: sprint-11 retro AI #5 (`production/retrospectives/retro-sprint-11-2026-05-08.md` line 158) — originating mandate; sprint-9 retro AI #2 (`production/retrospectives/retro-sprint-9-2026-05-07.md`) — ≥4 threshold codification source; sprint-12 plan S12-09 row (`production/sprints/sprint-12.md`) — acceptance criteria; companion lint precedent S12-04 `tools/ci/lint_story_status_consistency.sh` (CI sequencing model); `.claude/rules/tooling-gotchas.md` TG-3 — flag/next pattern reference for awk section extraction. **AI seed for sprint-12 retro**: NEW #13 candidate — anchored-regex-extraction discipline in prose-text scanning lints. **Original byte count**: ~440 bytes per-story changelog target (truncated to ~140-byte line in YAML).

### S12-07 — Closure-mode sprint planning pattern adoption (HYBRID; ≥3-of-5 signal trigger)

- 2026-05-09 — **done**: sprint-11 retro AI #6 closed via binding process-decision artifact at `production/decisions/closure-mode-sprint-pattern-2026-05-09.md` (~156 lines per `docs/process/decisions-convention.md` Route c template; all 9 required sections present; 4 reactivation triggers each with Signal + Required action sub-fields). **Decision**: closure-mode sprints (defined as ≥80% closure / admin / codification work; ≤20% greenfield) trigger when ≥3 of 5 signals evaluate TRUE at `/sprint-plan` invocation time. **Signal definitions**: (A) carryover concentration — pre-sprint-(N+1) carryover backlog contains ≥4 items OR any item with ≥4-time-carryover count per S12-06 USER-OWNED 5th-time threshold codification candidate; (B) unpaid codification debt — sprint-N retrospective contains ≥1 codification AI marked DEFERRED / NOT-CLOSED / CARRY-FORWARD; (C) recent epic-cluster close — ≥3 epics flipped to Status=Complete in `production/epics/index.md` within the last 2 sprints (sprint-(N-1) + sprint-N window); (D) hygiene drift accumulation — at least one of (i) ≥5 TODOs in `src/` per S11-11 audit threshold, (ii) ≥10 lint-drift items surfaced by `lint_story_status_consistency.sh`, OR (iii) ≥1 lint with vacuous-pass condition where forward-looking enforcement coverage is needed (e.g., S12-S03 migration-purity lint as of 2026-05-09); (E) no fresh must-have feature ready — at sprint-(N+1) plan time, GDD backlog has zero items where `/quick-design` OR `/create-stories` has produced ready-for-/dev-story files for a Pillar 1-4 feature. **Action when fired**: count ≥3 → producer designates closure-mode + ÷4 multiplier; count = 2 → borderline-mixed (closure-leaning permitted with documented rationale); count ≤1 → mixed-mode binding designation + greenfield-leaning mix appropriate. **Override authority**: producer (claude in this project's collaboration model) retains override authority at sprint plan time; override is documented in NEW `## Sprint Mode` section. **Reactivation triggers** (4): (T1) RECURRING — every sprint-(N+1) `/sprint-plan` invocation runs the signal evaluation; (T2) ≥2 producer overrides in any rolling 3-sprint window triggers signal re-calibration via retro AI; (T3) velocity-multiplier model amendment (sprint-10 retro AI #4's ÷3 mixed / ÷4 closure / ÷5 greenfield calibration) re-validates §3.6 cost-benefit; (T4) companion edit — `.claude/skills/sprint-plan/SKILL.md` Phase 2 template gains NEW required `## Sprint Mode` section at first post-decision `/sprint-plan` invocation (Trigger 4 deferred to sprint-13 plan to avoid scope-creep at S12-07). **Why HYBRID over EMBRACE-as-cadence**: cadence-based ignores codification debt timing — sprint-11 over-performance multiplier (÷4.4) was contingent on signal density (sprint-11 fired exactly 4 of 5 signals); cadence would force closure-mode into ≤2-signal sprints leaving multiplier theoretical. **Why HYBRID over RESERVE-as-stopgap**: stopgap framing misses 4 of 5 signals that justified sprint-11 (only carryover threshold was NOT a sprint-11 trigger; the other 4 signals were dominant); stopgap-only would under-allocate closure-mode to ~10-15% of sprints when ~20-25% is empirically supported. **Why ≥3-of-5 threshold**: ≥4 would be at sprint-11's empirical floor (fragile if any signal weakens); ≥2 fires too often (~40% of sprints retroactively); ≥3 places threshold at the empirical signal-density that distinguished sprint-11 from sprint-7→10 (≤2 signals each). **Decision retroactive scope**: sprint-12 was planned mixed-mode at 2026-05-08; this decision binds sprint-13+ planning ONLY. **Cross-references** (15+): originating retro line (sprint-11 retro §"What Went Well" item 1 + §"Velocity Analysis" + §"Sprint-12 Action Items" #6); originating sprint task (sprint-status.yaml line 31); supporting velocity baselines (sprint-9/10 retro AI #4); convention compliance (`docs/process/decisions-convention.md` §3 template + §4 trigger discipline + §7 skill-route meta-decision); companion future edit (`.claude/skills/sprint-plan/SKILL.md` Phase 2 template `## Sprint Mode` addition); reactivation monitoring (per-sprint /sprint-plan + retro time + skill template); companion sprint-12 decisions (pillar-3-deferral + prototype-as-gate-sufficient — gate-check path-to-PASS items 3a + 3b). **Meta-observation surfaced by S12-07**: `production/decisions/` now contains **4 artifacts** across **2 distinct sprints** (ci-lane-gap from sprint-10; pillar-3-deferral + prototype-as-gate-sufficient + closure-mode-sprint-pattern from sprint-12). Per `docs/process/decisions-convention.md` §7 skill-promotion trigger ("≥3 decision artifacts AND ≥2 distinct sprint origins"), the trigger has **FIRED** at this sprint-12 close-out. **Sprint-12 retro AI candidate (NEW #12)**: skill-promotion evaluation for `production/decisions/` convention — promote to sibling `/decision-record` skill scaffolding the Header block + 9 required sections + trigger format from convention doc; convention doc becomes skill `SKILL.md` reference. NOT closed by this S12-07 decision; tracked separately. **Test baseline preserved**: 1273/1273 PASS / 0 errors / 0 failures / 0 flaky / 0 skipped / 0 orphans / Exit 0; 130/130 test suites; 18.6s wall time; **60th consecutive failure-free baseline** (was 59th at S12-05 close). Doc-only addition — no source code change, no test count change. **Sprint-12 progression delta**: Must 2/3 done (S12-01 + S12-02); Should 2/3 done (S12-04 + S12-05); Nice-claude 1/3 done (S12-07 this); USER-OWNED 0/2. **Hygiene streak**: 44 in-patch sprint-status closes (was 43 at S12-05 close; +1 this story). **AI seed for sprint-12 retro**: (a) NEW #12 — `production/decisions/` skill-promotion trigger fired (per convention §7); (b) refinement of #6 — Trigger 1 (recurring per-sprint signal evaluation) becomes the standing application of this decision; ≥3 firing sprints across sprint-13/14/15 will retro-validate the calibration. **Original byte count**: ~470 bytes per-story changelog target (truncated to ~140-byte line in YAML).

### S12-05 — TODO triage Address actions bundle (5 → 2)

- 2026-05-09 — **done**: sprint-11 retro AI #4 + S11-11 process-audit (`production/process-audits/todo-triage-2026-05-08.md`) follow-through. 5 src/ TODOs surveyed + 4 surgical edits applied across 3 files; 1 TODO genuinely deferred unchanged. **Disposition**: (TODO-01) `map_grid.gd:915` Dijkstra heuristic — KEEP as-is; well-formed perf-defer with ADR-0004 §Decision 7 + TR-map-grid-006 refs; correctness unaffected; only matters at 40×30 m=10 benchmark scale. (TODO-02) `save_manager.gd:200` "(migration shim — see TODO below)" — REMOVE stale phrase; migration was implemented at line 210 via `SaveMigrationRegistry.migrate_to_current(ctx)` during save-load epic story-002 (closed 2026-05-08); replaced docstring step "6. Return ctx (migration shim — see TODO below)" with accurate steps "6. SaveMigrationRegistry.migrate_to_current(ctx) — apply schema migrations / 7. GameBus.save_loaded.emit(migrated); return migrated". (TODO-03) `payloads/beat_cue.gd:6` `## TODO:` — REFORMAT to `## NOTE:`; stub-class file (only `class_name BeatCue extends Resource`); future-Story-Event-GDD-#10 marker is informational not actionable; reformat drops it from `grep TODO` count. (TODO-04) `grid_battle_controller.gd:342` `# TODO(story-003+):` — REWORD; story-003+ ref stale (story shipped); accurate ref is Battle AI ADR (sprint-7+; NOT YET WRITTEN) per ADR-0014 line 598 + GDD §AI awareness lines 462/482/508 + tr-registry.yaml TR-grid-battle-controller-006 + architecture.yaml line 1202; new wording: `# TODO (Battle AI ADR — not yet written; sprint-7+): populate FSM state, unit positions, acted flags. Shape locked at ADR authoring time per ADR-0014 §10 Key Interfaces (opaque shape) + ADR-0014 line 598.` (TODO-05) `grid_battle_controller.gd:424` `# TODO(story-006): reset per-turn acted flag for this unit` — REMOVE; fully stale; the function below the comment does AI-action emission (story-006 shipped that), NOT acted-flag reset which happens in `end_player_turn()` at line 357 (`_acted_this_turn.clear()`) and the round_started handler. Comment didn't describe what the function actually does — leftover from a discarded design. The next-line comment "Per ADR-0019 + grid-battle.md CR-3: AI-turn detection + ai_action_requested emission." is the correct doc-comment + remained intact. **Result**: TODO count 5 → 2 ✓ matches sprint-status.yaml S12-05 spec target ("post-cleanup TODO count 5→2"). The 2 remaining TODOs are both genuine future-work deferrals with concrete ADR + TR + future-epic refs (TODO-01 = perf-defer to TR-map-grid-006 benchmark; TODO-04 = Battle AI ADR-defer to sprint-7+ unwritten ADR). **Process-audit S11-11 disposition reconciliation**: S11-11 audit document classified TODO-04 as "Address (sprint-12 candidate: get_battle_state_snapshot stub unused-by-AISystem)" + TODO-05 as "Address (sprint-12 candidate: stale per-turn-acted-flag TODO superseded by round-end bulk-clear)" + TODO-01 + TODO-03 as "Defer-with-context" + TODO-02 as "Remove (stale doc reference)". Actual S12-05 dispositions: TODO-04 KEEP-with-reword (audit's "Address" framing was speculative — Battle AI ADR is genuinely unwritten, so the stub is correctly deferred until ADR authoring locks the shape; reword instead of remove); TODO-05 REMOVE (matches audit); TODO-01 KEEP (matches audit); TODO-03 REFORMAT (TODO→NOTE; matches audit's "Defer-with-context" intent by signalling informational not actionable); TODO-02 REMOVE-as-stale-phrase (matches audit). Net: 4 of 5 audit dispositions matched; 1 refined (TODO-04 stub kept rather than populated since the consumer ADR doesn't exist yet — consumer-shape-defines-stub-shape principle). **Test baseline preserved**: 1273/1273 PASS / 0 errors / 0 failures / 0 flaky / 0 skipped / 0 orphans / Exit 0; 130/130 test suites executed; **59th consecutive failure-free baseline** (was 58th at S12-04 close). Comment-only changes — no logic affected, no test count change, no test additions warranted (TODOs are documentation hygiene). **Sprint-12 progression delta**: Must 2/3 done (S12-01 save-load decomposition + S12-02 chapter-prototype-demo); Should 2/3 done (S12-04 + S12-05 this); Nice-claude 0/3; USER-OWNED 0/2. **Hygiene streak**: 43 in-patch sprint-status closes (was 42 at S12-04 close; +1 this story). **Cross-references**: `production/process-audits/todo-triage-2026-05-08.md` S11-11 source audit; `.claude/rules/godot-4x-gotchas.md` no new gotcha (comment-only edits don't surface new GDScript patterns); `docs/tech-debt-register.md` no new TD entry (TODO triage is hygiene cleanup, not technical debt accumulation). **AI seed for sprint-12 retro**: process-audit-to-execution disposition matching at 4-of-5 (80%) — strong signal for the audit framework value; the 1 mismatch (TODO-04) refines the audit's classification rule: "stubs awaiting downstream-ADR-defined consumer shape should default to KEEP (with stale-ref reword) rather than Address, until the consumer ADR lands." **Original byte count**: ~270 bytes per-story changelog target (truncated to ~167-byte line in YAML).

### S12-04 — `lint_story_status_consistency` 33-drift bulk cleanup + CI wire

- 2026-05-08 — **done**: sprint-11 retro AI #3 follow-through; bulk-cleanup of 33 stale Status entries detected by `tools/ci/lint_story_status_consistency.sh` (codified at sprint-11 S11-03 2026-05-08 as root-cause fix for the 4-invocation BACKFILL CLOSE-OUT pattern caught at S10-04 + S11-02). **Drift breakdown** (from baseline lint output): 21 failures of `index.md row status=Ready` while story Complete + 12 failures of `EPIC.md Status header=Ready while all stories in epic are Complete (epic-terminal closure not propagated)`. **Categorisation discovery**: 33 individual story drifts collapsed to **4 unique fixes** at the canonical-source level — (1) `balance-data/EPIC.md:6` header Ready → Complete (2026-05-01) propagating to 5 stories; (2) `turn-order/EPIC.md:6` header Ready → Complete (2026-05-02) propagating to 7 stories; (3) `production/epics/index.md` damage-calc row (line 19) Stories cell `Not yet created — run /create-stories damage-calc` → `**11/11 Complete** 🎉 (epic closed 2026-04-27)` + Status cell `**Ready** (2026-04-26)` → `**Complete** (2026-04-27) 🎉` propagating to 11 stories; (4) `production/epics/index.md` input-handling row (line 25) Stories cell `Not yet created — run /create-stories input-handling` → `**10/10 Complete** 🎉 (epic-terminal close 2026-05-07 Sprint 9 S9-05)` + Status cell `**Ready** (2026-05-02)` → `**Complete** (2026-05-07) 🎉` propagating to 10 stories. **CI wiring same-patch**: appended `Lint story Status consistency across 4 canonical sources` step to `.github/workflows/tests.yml` after the 3 save-load story-003 lint steps (sequence: `lint_save_resource_loader_cache_mode_ignore.sh` → `lint_save_migration_callable_purity.sh` → `lint_save_context_export_discipline.sh` → `lint_story_status_consistency.sh`). **Verification**: pre-fix lint exit 1 with 33 STATUS_CONSISTENCY_FAIL blocks; post-fix lint exit 0 with `PASS — all Complete stories have consistent downstream Status`. **Full test suite**: 1273/1273 PASS / 0 errors / 0 failures / 0 flaky / 0 skipped / 0 orphans / Exit 0; 130/130 test suites executed; 19s 045ms wall time; 58th consecutive failure-free baseline preserved. **Pattern note**: lint's `EPIC.md table row` regex requires `**Bold**` Status text but balance-data + turn-order Stories tables contain unbolded `Ready` entries — these are NOT detected by the lint (`EPIC_TABLE_STATUS=""` skips the check) and remain stale at row level. Out-of-scope for this S12-04 surgical-fix pass per "33-drift bulk cleanup" framing; could be addressed via an EPIC.md-table-row hygiene sweep at sprint-12 retro time if desired. **Sprint-12 progression delta**: Must 2/3 done (S12-01 save-load decomposition + S12-02 chapter-prototype-demo); Should 1/3 done (S12-04 this); Nice-claude 0/3; USER-OWNED 0/2. **Hygiene streak**: 42 in-patch sprint-status closes (was 41 at S12-02 close; +1 this story).

### S12-02 — Pillar 4 atmospheric moment / chapter-prototype-demo epic-terminal close

- 2026-05-08 — **SHIPPED**: `chapter-prototype-demo` Demo-epic 1/1 epic-terminal close — first Demo-layer epic in project. **Implementation commits**: `f6b14e6` (`/quick-design` spec) → `e5689d3` (`/create-stories` epic stub + story-001 + index.md) → `f577345` (`/story-readiness` 2 gap fixes Estimate field + Memory-impact 10KB→414 KiB) → `aa55969` (`/dev-story` impl: `chapter.gd` +126 LoC + `REPORT.md` +4 LoC + `atmospheric_moment_test.gd` NEW 445 LoC) → THIS COMMIT (`/story-done` close-out). **AC coverage**: 7/7 PASS — agent split AC-S12-02-1..6 into 7 granular tests instead of 1 composite (better failure isolation; aligns with story's QA Test Cases table). **Test progression**: 1266 → 1273 (+7); 1273/1273 PASS / 0 fail / 0 error / 0 flaky / 0 skipped / 0 orphans; **57th consecutive failure-free baseline** (was 56th at save-load story-003 epic-terminal). 130/130 test suites executed. **Pillar 4 atmospheric layer mechanically observable for first time**: REWRITTEN branch resolution dispatches reserved-color treatment (주홍 `#C0392B` panel tint at 0.35α via ColorRect overlay tween 0.4s easeOut → 1.5s dwell hold → 0.2s easeIn fade-out + 금색 `#D4A017` title color via `add_theme_color_override`) + synthesized 묵 hum audio cue (220Hz fundamental + 330Hz harmonic at -6dB, 1.2s envelope: ease-in 0..0.2s + sustain 0.2..0.8s + ease-out 0.8..1.2s, ~414 KiB pre-baked `PackedVector2Array` via `AudioStreamGenerator` + `AudioStreamGeneratorPlayback.push_buffer`) + 1.5s ceremonial dwell lockout per AC-SP-9 (RetryButton + QuitButton both `disabled = true` during envelope). Other 3 branches (DEFEAT / HISTORICAL / PARTIAL) unchanged — no overlay, no audio, no dwell — regression-tested 3 ways. Defensive reset on `_on_retry()` (overlay alpha 0 + audio stop + button re-enable + `_last_branch = ""`) prevents stacked dispatches. **Gate-check 2026-05-08 path-to-PASS item 3 CLOSED** (≥1 player-facing Pillar 3/4 demonstration). **AI #9 codification ratchet** (sprint-12 retro candidate): `godot-gdscript-specialist` mid-execution stall pattern stable at **4 invocations** (was 2 at sprint-12 entry; +2 this story — first-spawn stall partway through investigation + continuation-spawn stall after writing test + reporting test count). Final-report from continuation agent had hallucinated `src/chapter_prototype/atmospheric_moment.gd` path; `git status` verification revealed actual files correctly prototype-scoped. **Codification strengthened**: pre-authorize end-to-end (already in retro AI #9 from save-load story-002/003) + verify agent's final-report claims against `git status` before trusting narration. **Process precedents**: (1) first prototype-side `/quick-design` → `/create-stories` adaptation chain — when design source is a quick-spec NOT an epic, create a minimal Demo-layer epic stub at `production/epics/chapter-prototype-demo/EPIC.md` so the story rolls up to standard epic structure + index; (2) first Demo-layer epic in project — establishes layer-classification precedent for prototype-isolated work that doesn't fit Foundation/Core/Feature/Presentation taxonomy. **Spec deviations** (4 advisory; none harmful; no tech-debt entries warranted): (a) test count +7 instead of spec's +1 — better coverage; (b) `chapter.tscn` not edited despite being in story's Affected Systems table — file is empty (Control root + script attach only), all UI is programmatic per existing chapter.gd convention, atmospheric ColorRect + AudioStreamPlayer added programmatically in `_ready()`; (c) `AudioStreamGenerator.push_buffer()` bool return not checked at call site — sample-drop indicator, acceptable for prototype per `.claude/rules/prototype-code.md` relaxed standards, tighten in production-port; (d) `tests/integration/chapter_prototype/` directory created — was implied by Test Evidence path, not explicitly listed in scope. **`lint_story_status_consistency` baseline**: 33 pre-existing drifts at story close (separate sprint-12 S12-04 Should-Have bulk-cleanup task per sprint-11 retro AI #3); my close-out adds 0 new drifts (chapter-prototype-demo/story-001 + sprint-status.yaml + EPIC.md + index.md all atomically aligned). **Streak**: 41 in-patch sprint-status closes (was 40 at save-load story-003 close; +1 this story). **Cross-pillar enabling effects**: Pillar 4 atmospheric layer can now bind to production scenario runner when authored (deferred); REPORT.md Pillar 4 verdict upgraded "substrate present" → "substrate present + atmospheric demonstration shipped". **Code-review verdict**: APPROVED via lean manual review (per prototype-relaxed standards + AI #9 mitigation; specialist spawn deliberately skipped to avoid 5th stall risk; documented inline at conversation 2026-05-08). **Original byte count**: ~298 bytes per-story changelog + ~1500 bytes top-level updated (truncated to 173 + 191 bytes pointer respectively in YAML).

...
## full alpha; others recede to _UNIT_LABEL_DIM_ALPHA (0.45).
##
## Coverage:
...
func test_refresh_with_no_active_or_selected_makes_all_labels_dim() -> void:
	...
	assert_float(_name_label_alpha(_player_parent, 2, 3)).is_equal_approx(0.45, 0.001)
	assert_float(_name_label_alpha(_player_parent, 4, 5)).is_equal_approx(0.45, 0.001)
	assert_float(_name_label_alpha(_enemy_parent, 6, 3)).is_equal_approx(0.45, 0.001)
...
func test_s43_alpha_constants_are_in_expected_range() -> void:
	...
	assert_float(dim).override_failure_message(
		"S43: dim alpha must be between 0.2 and 0.8 to stay legible"
	).is_greater(0.2)
	assert_float(dim).is_less(0.8)

...
## Session-33 — Production shu_canon_main.json ch07 record carries the ESCORT retrofit.
## Verifies the chapter JSON parses + ch07 victory_conditions hydrates correctly.
func test_shu_canon_main_ch02_carries_escort_target_0() -> void:
	var json_text: String = FileAccess.get_file_as_string("res://assets/data/scenarios/shu_canon_main.json")
	...
	assert_bool(ch07_record.is_empty()).override_failure_message(
		"shu_canon_main.json must contain ch07_changban_bridge record"
	).is_false()
...
## Session-34 — Production shu_canon_main.json ch08 record carries the REACH_TILE retrofit.
...
func test_shu_canon_main_ch03_carries_reach_tile_target_13_4() -> void:
	var json_text: String = FileAccess.get_file_as_string("res://assets/data/scenarios/shu_canon_main.json")
	assert_bool(json_text.is_empty()).is_false()
	var parsed: Variant = JSON.parse_string(json_text)
	assert_object(parsed).is_not_null()
	...
	assert_bool(ch08_record.is_empty()).override_failure_message(
		"shu_canon_main.json must contain ch08_xiakou_outskirts record"
	).is_false()
...
## Production shu_canon_main.json ch10 record carries the SURVIVE_N_ROUNDS=5 retrofit.
...
func test_shu_canon_main_ch05_carries_survive_5_rounds() -> void:
	var json_text: String = FileAccess.get_file_as_string("res://assets/data/scenarios/shu_canon_main.json")
	assert_bool(json_text.is_empty()).is_false()
	var parsed: Variant = JSON.parse_string(json_text)
	assert_object(parsed).is_not_null()
	...
	assert_bool(ch10_record.is_empty()).override_failure_message(
		"shu_canon_main.json must contain ch10_chibi_main record"
	).is_false()
...
# ─── Phase B (shu_canon_main ch11~ch14) — 형주 4군 평정 + 통합 authoring sanity ────────


## Phase B regression sentinel — shu_canon_main must declare exactly 14 chapters
...
func test_shu_canon_main_phase_b_authors_ch11_to_ch14_with_wei_yan_defection_chain() -> void:
	var json_text: String = FileAccess.get_file_as_string("res://assets/data/scenarios/shu_canon_main.json")
	assert_bool(json_text.is_empty()).override_failure_message(
		"shu_canon_main.json must exist at assets/data/scenarios/shu_canon_main.json"
	).is_false()
	var parsed: Variant = JSON.parse_string(json_text)
	var data: Dictionary = parsed as Dictionary
	var chapters: Array = data["chapters"] as Array
	assert_int(chapters.size()).override_failure_message(
		"Phase E: shu_canon_main must declare exactly 25 chapters — 영걸전식 풀 캠페인 완성 (A 10 + B 4 + C 3 + D 5 + E 3)"
	).is_equal(25)
	...
	for expected_id: String in [
		...
		assert_bool(by_id.has(expected_id)).override_failure_message(
			"Phase B: shu_canon_main missing chapter_id '%s'" % expected_id
		).is_true()
...
## Phase B end-to-end hydration smoke — full shu_canon_main loads through
...
func test_shu_canon_main_phase_b_full_scenario_loads_via_runner_without_fault() -> void:
	var runner: Node = ScenarioRunnerTestSeam.make_isolated_runner()
	auto_free(runner)

	var ok: bool = runner.load_scenario("res://assets/data/scenarios/shu_canon_main.json")

	assert_bool(ok).override_failure_message(
		"Phase B/C: ScenarioRunner.load_scenario MUST succeed on full shu_canon_main.json"
	).is_true()


# ─── Phase C (shu_canon_main ch15~ch17) — 익주 입성 authoring sanity ──────────────────
...
func test_shu_canon_main_phase_c_authors_ch15_to_ch17_with_pang_tong_survival_chain() -> void:
	var json_text: String = FileAccess.get_file_as_string("res://assets/data/scenarios/shu_canon_main.json")
	var parsed: Variant = JSON.parse_string(json_text)
	var data: Dictionary = parsed as Dictionary
	var chapters: Array = data["chapters"] as Array
	...
	for expected_id: String in [
		...
		assert_bool(by_id.has(expected_id)).override_failure_message(
			"Phase C: shu_canon_main missing chapter_id '%s'" % expected_id
		).is_true()
...
# ─── Phase D (shu_canon_main ch18~ch22) — 한중·이릉·영걸전 시그니처 분기 3개 ────────────
...
func test_shu_canon_main_phase_d_authors_ch18_to_ch22_with_three_signature_branches() -> void:
	var json_text: String = FileAccess.get_file_as_string("res://assets/data/scenarios/shu_canon_main.json")
	var parsed: Variant = JSON.parse_string(json_text)
	var data: Dictionary = parsed as Dictionary
	var chapters: Array = data["chapters"] as Array
	...
	for expected_id: String in [
		...
		assert_bool(by_id.has(expected_id)).override_failure_message(
			"Phase D: shu_canon_main missing chapter_id '%s'" % expected_id
		).is_true()
...
# ─── Phase E (shu_canon_main ch23~ch25) — 남만·북벌·오장원·영걸전 finale ──────────────
...
func test_shu_canon_main_phase_e_authors_ch23_to_ch25_with_qixing_revival_finale() -> void:
	var json_text: String = FileAccess.get_file_as_string("res://assets/data/scenarios/shu_canon_main.json")
	var parsed: Variant = JSON.parse_string(json_text)
	var data: Dictionary = parsed as Dictionary
	var chapters: Array = data["chapters"] as Array
	...
	for expected_id: String in [
		...
		assert_bool(by_id.has(expected_id)).override_failure_message(
			"Phase E: shu_canon_main missing chapter_id '%s'" % expected_id
		).is_true()
...
## prose 존재 검증.
func test_shu_canon_main_ch25_authors_ending_screen_text_keys_for_all_four_branches() -> void:
	var json_text: String = FileAccess.get_file_as_string("res://assets/data/scenarios/shu_canon_main.json")
	var data: Dictionary = JSON.parse_string(json_text) as Dictionary
	var ch25: Dictionary = {}
	for c: Variant in (data["chapters"] as Array):
...
func test_shu_canon_main_ch25_authors_legendary_destiny_tier() -> void:
	var json_text: String = FileAccess.get_file_as_string("res://assets/data/scenarios/shu_canon_main.json")
	var data: Dictionary = JSON.parse_string(json_text) as Dictionary
	var ch25: Dictionary = {}
	for c: Variant in (data["chapters"] as Array):
...
## 25-chapter master plan completion sentinel — shu_canon_main must declare exactly 25 chapters
...
func test_shu_canon_main_full_campaign_25_chapter_progression_complete() -> void:
	var json_text: String = FileAccess.get_file_as_string("res://assets/data/scenarios/shu_canon_main.json")
	var data: Dictionary = JSON.parse_string(json_text) as Dictionary
	var chapters: Array = data["chapters"] as Array

	assert_int(chapters.size()).override_failure_message(
		"25-chapter master plan: shu_canon_main MUST declare exactly 25 chapters"
	).is_equal(25)
...
# ─── Multi-step survival cascade — shu_canon_main.json data sentinel (S65) ───────────


## After S65 cascade rollout, shu_canon_main.json must declare:
...
func test_shu_canon_main_signature_branches_root_and_cascade_overrides_complete() -> void:
	var json_text: String = FileAccess.get_file_as_string("res://assets/data/scenarios/shu_canon_main.json")
	var data: Dictionary = JSON.parse_string(json_text) as Dictionary
	...
	assert_int(sig_keys.size()).override_failure_message(
		"shu_canon_main root signature_branches must declare 5 영걸전 시그니처 키"
	).is_equal(5)

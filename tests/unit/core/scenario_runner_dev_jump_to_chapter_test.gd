...
##
## Covers:
##   - Valid jump to ch06..ch10 of shu_canon_main + mvp_wei
...
func test_dev_jump_lands_at_each_shu_canon_main_chapter() -> void:
	# 25 chapters in shu_canon_main — 영걸전식 풀 캠페인 완성 (도원결의 → 오장원).
	...
	for idx: int in expected_ids.size():
		...
		assert_bool(ok).override_failure_message(
			"dev_jump_to_chapter(shu_canon_main, %d) must succeed (returned false)" % idx
		).is_true()
		...
		assert_str(ch.chapter_id).override_failure_message(
			"dev jump to shu_canon_main[%d] landed on '%s', expected '%s'"
...
func test_dev_jump_rejects_overflow_index() -> void:
	# Phase E: shu_canon_main has exactly 25 chapters (영걸전식 풀 캠페인 완성), index 25 is out of range.
	var runner: Node = ScenarioRunnerTestSeam.make_isolated_runner()
	auto_free(runner)

...
##
## Integration with shu_canon_main.json (cascade ch20 관우 생존 → ch25까지 roster 유지)
## is covered separately in scenario_runner_victory_conditions_hydration_test.gd
## via _shu_canon_main_full_campaign_persistent_cascade_test (added same session).
...
## _signature_branch_keys when present. shu_canon_main.json declares all 5 영걸전
## signature keys post-S65.
func test_load_scenario_pulls_signature_branches_from_shu_canon_main_root() -> void:
	...
	assert_bool(ok).override_failure_message("shu_canon_main.json must load").is_true()
	var keys: PackedStringArray = runner._signature_branch_keys
	assert_int(keys.size()).override_failure_message(
		"shu_canon_main signature_branches must declare 5 영걸전 시그니처 키 (위연/방통/관우/장비/유비)"
	).is_equal(5)
...
## (mvp_wei.json does NOT declare cascades — only shu_canon_main does.)
...
func test_restore_from_save_context_v2_repopulates_cascade_state() -> void:
	var runner: Node = ScenarioRunnerTestSeam.make_isolated_runner()
	auto_free(runner)
	# Load shu_canon_main (real scenario for real chapter_id resolution).
	var ok: bool = runner.load_scenario("res://assets/data/scenarios/shu_canon_main.json")
...
## (CR-3 outcome invariant). Verified via shu_canon_main ch25 hidden_condition relief:

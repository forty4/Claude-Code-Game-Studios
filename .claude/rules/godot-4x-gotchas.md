...
**Discovered**: pattern stable at **6 invocations** (advanced 4 → 6 within ~24hr at sprint-15 S15-J + S15-D 2026-05-10):
...
5. **POLISH-012 (sprint-15 S15-D /dev-story Phase 4 investigation, 2026-05-10 PM late-late)** — CLOSED-LOOP variant: even after the POLISH-011 absorption arc S15-A/B/C wired the 3 internal root causes, the production WIRING that activates them was missed. `set_action_controller` DI surface added by S15-A (commit `ab924aa`) had 0 production callers (`grep -rn "set_action_controller" src/ tests/` → 7 test sites + 0 src callers); production main_scene falls through TEST-SEAM no-op pass for T5; battle resolves to ROUND_CAP_DRAW identically with-or-without S15-A/B/C wiring. Caught at investigation-time BEFORE first test run (faster + cheaper than the anticipated test-first-run-failure path). Closed at S15-J via mid-sprint amendment commit `a4636dc` (single ~3-line wire-up in `battle_scene.gd` STEP 5 + new handler `_on_turn_runner_action_request` on `grid_battle_controller.gd` + ADR-0014 §Amendment 2026-05-10 (#3) + 6 integration tests).
6. **POLISH-013 META-pattern (sprint-15 S15-D /dev-story 3-spawn-cycle attempt, 2026-05-10 PM very-late)** — META: even the test designed to CLOSE this G-30 gap (the natural-loop integration test) ITSELF surfaces a NEW G-30 instance. 3 fixture iterations (chapter-1 / hybrid 1-stub-player+4-enemies / TRUE 0-player-units+4-enemies) all stalled identically at first `unit_turn_started` emit — 2 emits captured / 4000 frames consumed / no `victory_condition_detected` / no `ai_action_requested`. AISystem CONNECT_DEFERRED subscriber never fired in test scope. Whether this indicates test-env-only gap (production fine; needs S15-G windowed re-attestation to confirm) OR real production defect (POLISH-011 not actually closed) is UNRESOLVED. S15-D DEFERRED to sprint-16; S15-J wiring test #2 verifies enemy-side dispatch chain at UNIT scope (component contracts correct; cross-system progression unverified). Codified as META-pattern: test infra alone may not be sufficient for G-30 mitigation; structural review at sprint-15 retro should evaluate input simulation + per-unit T5 timeouts + dedicated G-30 mitigation EPIC as alternatives.

**Source-story trace**: sprint-13 retro AI #6 + sprint-14 plan S14-06 (`production/sprints/sprint-14.md:52`) + POLISH-011 TRIAGE FINDING block at `production/polish-backlog.md` (sprint-14 PM late-late triage 2026-05-09) + sprint-15 mid-sprint amendments at `production/sprints/sprint-15.md` (S15-J POLISH-012 closure commit `a4636dc` + S15-D POLISH-013 deferral commit `dca58a4`). Pattern stability advanced from 2 (sprint-13 retro framing) → 3 (sprint-14 entry POLISH-011 surfacing) → 4 (sprint-14 PM late-late POLISH-011 actual root cause re-attribution) → 5 (sprint-15 PM late-late POLISH-012 production-wiring residual) → 6 (sprint-15 PM very-late POLISH-013 META-pattern test-env-vs-prod gap) — all within ~72 hours total; sprint-15 alone advanced pattern stability +2 in <12 hours.
